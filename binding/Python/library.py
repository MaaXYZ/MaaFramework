...
class library:
    ...
    def open(cls, path: Union[pathlib.Path, str], toolkit: bool = True) -> bool:
        ...
        try:
            cls.framework_libpath = pathlib.Path(path) / platform_values[platform_type]['framework_libpath']
        ...
        if toolkit:
            try:
                cls.toolkit_libpath = pathlib.Path(path) / platform_values[platform_type]['toolkit_libpath']

...
2. **Sequential Detection:**  
   ...
   - A screenshot is taken before recognition; if every enabled node in `next` uses [`DirectHit`](#directhit), the screenshot is skipped  
...
```mermaid
    flowchart TB
        ...
        E --> F[post_delay]
        F --> Q{"all next nodes DirectHit?"}
        Q -->|yes| G[recognize next]
        Q -->|no| P[screenshot]
        P --> G
...
When every enabled node in the `next` list uses `DirectHit`, the framework skips the screenshot and runs the action directly. This allows non-visual tasks to continue when the controller is disconnected or cannot provide screenshots.

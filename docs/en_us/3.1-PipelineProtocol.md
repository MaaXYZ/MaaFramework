...
**Type**: *boolean*  
**Default**: `false`  
**Description**: Enables jump-back mechanism. When this node is recognized and executed, after all subsequent nodes in its chain have completed execution, the system will return to the parent node and continue attempting to recognize the parent node's `next` list.

**Execution Flow**:

1. The parent node recognizes nodes in the `next` list sequentially
2. If a node with `jump_back` attribute is recognized, execute that node and its subsequent node chain
...
- If D is recognized (assuming B and C are not), execute D and its subsequent node chain

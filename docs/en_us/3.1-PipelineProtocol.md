...
- `next` : *string* | *list<string, >*  
  ...
  - `.` Ignore node, treat as non-existent  
    For example: A: { next: [ ".B", "C" ]}, then "B" is ignored in this list, equivalent to A: { next: [ "C" ]}
...
- `is_sub`: *bool*  
    **(Deprecated in version 5.1, node prefix jump-back functionality `*` is recommended instead)**  
...
- `on_error` : *string* | *list<string, >*  
    ...
    💡 Supports the same node prefix functionality as `next` (`.` ignore, `*` jump-back).

...
A regular node list looks like this:
...
Since v5.1, node attributes functionality has been introduced, supporting two ways to set node attributes:
...
- Object form
    ...
    ```jsonc
    {
        "A": {
            "next": [
                "B",
                {   // Use object to set jump_back attribute for node C in this next list
                    "name": "C",
                    "jump_back": true,
                },
                "D"
...
- Prefix form
    ...
    ```jsonc
    {
        "A": {
            "next": [
                "B",
                "[JumpBack]C",  // Use `[]` attribute prefix to set jump_back attribute directly
                "D"
            ]
        }
    }
...
Both methods are equivalent, choose based on your development preference~

Currently available node attributes:

- `jump_back` / `[JumpBack]`  
    If this node is matched, after all subsequent nodes have completed execution, jump back to this node to try recognition again.  
    For example: A: { next: ["B", "C", "[JumpBack]D", "E"] }  
    When B, C are not recognized but D is recognized, D and D.next will be fully executed. But when D's pipeline is fully completed, it will return to node A again and continue to try to recognize B, C, D, E.  
    ...
    (Similar to the old version `is_sub`, the difference is that only the node in the current next list is treated as `is_sub`.)

    ```mermaid
    graph LR
        A -->|recognize| B
        A -->|recognize| C
        A -->|recognize and hit| D
        A -->|not performed| E
        D -->|execute subsequent tasks normally| F[D.next...]
        F -->|all completed, jump back| A
    ```

- More node attributes are under development, stay tuned!

...
> If a Win32 controller has declared a managed key domain through `MaaControllerPostSetBackgroundManagedKeys`, matching keys automatically route through the background guardian path. If a matching key is already being held by the guardian, `ClickKey` fails.
...
> [!NOTE]
>
> If a Win32 controller has declared a managed key domain through `MaaControllerPostSetBackgroundManagedKeys`, matching keys automatically route through the background guardian path. If a matching key is already being held by the guardian, `LongPressKey` fails.
...
> [!NOTE]
>
> If a Win32 controller has declared a managed key domain through `MaaControllerPostSetBackgroundManagedKeys`, matching keys automatically route through the background guardian path and continue to be corrected while the controller is idle.
...
> [!NOTE]
>
> If a Win32 controller has declared a managed key domain through `MaaControllerPostSetBackgroundManagedKeys`, matching keys automatically route through the background guardian path.

...
graph LR
    subgraph Setting Anchor
        A[Node A<br/>anchor: X] -->|executed successfully| X[Anchor X]
        B[Node B<br/>anchor: X] -->|executed successfully, overwrites| X
    end
    subgraph Referencing Anchor
        C[Node C] -->|recognize| D[Node D]
        C -->|recognize| E["[Anchor]X"]
        E -->|resolves at runtime| X
        X -->|if A executed last| RA[Execute Node A]
        X -->|if B executed last| RB[Execute Node B]
    end
```

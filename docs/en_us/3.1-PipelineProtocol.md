...
3. **Node Attribute Resolution (`NodeAttr`)**  
   - Nodes in `next` / `on_error` can carry attributes (such as `[JumpBack]`, `[Anchor]`)
   - For `[Anchor]` references, runtime resolves the anchor to a target node first; if the anchor is unset, the candidate node is skipped

4. **Hit and Action Execution**  
...
5. **Routing and State Update**
   - Route by action result after `run_action` returns:
     - If action succeeds: enter the matched node's `next` list
     - If action fails: enter the matched node's `on_error` list

   - After action returns, the matched node's `anchor` definitions are updated (regardless of action success/failure)

6. **`JumpBack` Return**
   - If the matched node is marked `jump_back` in the parent list, a parent jump-back point is recorded
   - When the branch finishes with `next` empty and the flow is not in error-handling path, control returns to the parent node and continues its `next`

7. **Error Handling and Interruption**
   - If all nodes in the current `next` list miss, retry by `rate_limit` until hit or timeout
   - If there is no executable candidate in this round of `next` (for example all disabled, or anchors unresolved), it fails directly into error handling without timeout retry
   - When `need_to_stop` becomes true (for example external `post_stop` or `StopTask`), the task loop is interrupted

### Termination Conditions

The task process terminates when any of the following conditions are met:

- Normal end: no executable successor remains (`next` is empty and the `JumpBack` stack is empty)
- Active stop: `need_to_stop == true` (for example `StopTask` / `post_stop`)
- Error end: after entering error handling, no continuation path remains (for example empty `on_error`, or no executable candidate / timeout during error handling; such unrecovered error-handling flow does not trigger `JumpBack`)
...
#### Case 3 (Action Failure)

```mermaid
    graph LR
        A[Recognize and hit node X] --> B[Execute X.action]
        B --> C{Action succeeded?}
        C -->|yes| D[Enter X.next]
        C -->|no| E[Enter X.on_error]
        E --> F{Can on_error continue?}
        F -->|yes| G[Continue subsequent flow]
        F -->|no| H[Task Failed]
```
...
| v5.9 | String-form `roi` / `target` added `[Anchor]AnchorName` reference support; fixed inconsistent action-failure follow-up behavior with/without `[JumpBack]` (error-handling path does not trigger jump-back) |
...
- `timeout`: *int*  
    ...
    Once any node is hit, the remaining detections in the current `next` list are skipped immediately and the flow enters the action phase.

- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    When the current node fails (e.g. recognition timeout, action execution failure), the nodes in this list are executed next. Optional, empty by default.  
    If this list is empty, node failure goes directly into the error-termination path.  
    Note: While in error-handling state, `JumpBack` is not performed. Jump-back may still happen later only after a subsequent hit restores non-error state and `next` becomes empty.  
    💡 Since v5.1, nodes with attributes, NodeAttr objects, or heterogeneous arrays combining both are supported. See [Node Attributes](#node-attributes) for details.

- `anchor`: *string* | *list<string, >* | *object* **💡 v5.1**  
    Anchor name. Optional, default is empty.  
    After the node is recognized and hit, the anchor mapping is updated when `run_action` returns (regardless of action success/failure). Multiple nodes can set the same anchor name, and later execution overwrites earlier mapping.  
...
- `repeat`: *uint* **💡 v5.3**  
    ...
    A single failed action attempt does not abort the repeat loop early; the final node action result uses the last action attempt.
...
The lifecycle of a node is as follows (excluding cross-node controls such as `anchor` / `[JumpBack]`):
...
```mermaid
    graph TB
        ...
        B --> C[pre_delay]
        C --> S0{need_to_stop?}
        S0 -->|yes| T[task stopped]
        S0 -->|no| D[action]
        D --> S{need_to_stop?}
        S -->|yes| T
        S -->|no| M{repeat unfinished?}
        M -->|yes| N[repeat_wait_freezes]
        N --> O[repeat_delay]
        O --> S0
        M -->|no| E[post_wait_freezes]
        E --> F[post_delay]
        F --> R{final action succeeded?}
        R -->|no| K[on_error]
        R -->|yes| Q[start timer]
        Q --> P[screenshot]
        P --> G[recognize next]
...
> [!NOTE]
>
> This diagram focuses on the "single-node action + `next` recognition retry" main loop. Cross-node controls are:
> `anchor`: updated after the node is hit, when `run_action` returns (regardless of action success/failure).
> `[JumpBack]`: parent is recorded when hitting nodes with this attribute; jump-back happens only when `next` is empty and the flow is not in error-handling path.
> `need_to_stop`: checked before and after each action execution, so setting it during runtime may end the current node in the action phase.
...
After execution, the task is marked as stop requested (`need_to_stop`) and further recognition loops are interrupted.
...
In implementation, jump-back points are pushed onto a stack first, and are popped only when the current branch has `next` empty and is not in error-handling path.
If the flow is currently in error handling (`on_error` path), jump-back is not performed; only after a later hit restores non-error state can jump-back happen when `next` becomes empty.
...
3. After the node chain completes execution, return to the parent node only if the current branch is not in error-handling path
...
- After D's node chain completes, return to node A only if the current branch is not in error-handling path
...
graph LR
    ...
    F --> G{in error-handling path?}
    G -->|no| A
    G -->|yes| H[no jump-back]
```
...
If the anchor does not exist or has been cleared, this candidate node is skipped in the current recognition round.
Anchor write-back occurs after `run_action` returns, not before action execution.

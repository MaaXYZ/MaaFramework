...
```mermaid
graph LR
    subgraph Setting Anchor
        A[Node A<br/>anchor: X] -->|executed successfully| S1[Anchor X = Node A]
        B[Node B<br/>anchor: X] -->|executed successfully| S2[Anchor X = Node B]
    end
    subgraph Referencing Anchor
        C[Node C] -->|recognize| D[Node D]
        C -->|recognize| E["[Anchor]X"]
        E -->|resolves at runtime| F{Value of Anchor X?}
        F -->|if A executed last| A
        F -->|if B executed last| B
    end
```
...
- `order_by`: *string*  
    How the results are sorted. Optional, default is `Horizontal`  
    Possible values: `Horizontal` | `Vertical` | `Area` | `Length` | `Random` | `Expected`  
    You can use it with the `index` field.
...
- `order_by`: *string*  
    How the results are sorted. Optional, default is `Horizontal`  
    Possible values: `Horizontal` | `Vertical` | `Score` | `Random` | `Expected`  
    You can use it with the `index` field.
...
- `order_by`: *string*  
    How the results are sorted. Optional, default is `Horizontal`  
    Possible values: `Horizontal` | `Vertical` | `Score` | `Area` | `Random` | `Expected`  
    You can use it with the `index` field.  

...
- `recognition`: *string*
    ...
    Possible values: [`DirectHit`](#directhit) | [`Multiple`](#multiple) | [`TemplateMatch`](#templatematch) | [`FeatureMatch`](#featurematch) | [`ColorMatch`](#colormatch) | [`OCR`](#ocr) | [`NeuralNetworkClassify`](#neuralnetworkclassify) | [`NeuralNetworkDetect`](#neuralnetworkdetect) | [`Custom`](#custom)
...
Composite recognition. This node is recognized only when **all** sub-recognitions in `and` are recognized successfully on the **same screenshot frame** (logical AND).

- `and`: *list<object, >*
    Sub-recognitions array. **Required**.
    Each element can be either Pipeline v1-style recognition fields (e.g., `{ "recognition": "OCR", ... }`) or Pipeline v2-style recognition objects (e.g., `{ "type": "TemplateMatch", "param": { ... } }`). Mixed usage is allowed.

- `sub_name`: *string*
    Optional field for each sub-recognition element. If set, its result box can be referenced by later sub-recognitions via `roi: "sub_name"` (same syntax as referencing a node name).
    Notes:
  - If multiple sub-recognitions share the same `sub_name`, the **last** one in `and` wins for later references.
  - In `Multiple.and`, `roi: "xxx"` resolves in this order:
    1. Try to resolve `"xxx"` as a **previous** sub-recognition's `sub_name` (earlier in `and`). If found but it has no box, the ROI falls back to full screen.
    2. Otherwise, it behaves like `roi: "NodeName"` (resolve `"xxx"` as a previously executed node name). If it cannot be resolved, the ROI falls back to full screen.

- `box_index`: *int*
    Selects which sub-recognition's result box will be used as the overall recognition box. Optional, default is `0`.
    Must satisfy `0 <= box_index < and.length`, otherwise parsing should fail.
    If the selected sub-recognition has no box, the node is still considered recognized, but the overall box will be empty.
    If the overall box is empty and the action uses `target: true`, the target falls back to full screen (same as `[0, 0, 0, 0]`).

Pipeline v1 example (use `sub_name` + `roi: "sub_name"`):
...
Pipeline v2 example (mixed v1/v2 sub-items are allowed):
                ...
                "and": [
                    {
                        "sub_name": "icon",
                        "type": "TemplateMatch",
                        "param": { "template": "storage_icon.png", "threshold": 0.7 }
                    },
                    {
                        "recognition": "OCR",
                        "roi": "icon",
...
- `dy`: *int*  
    Vertical scroll distance, positive values scroll down, negative values scroll up. Optional, default is 0.  

> [!NOTE]
>
> - Adb Controller does not support scroll operations. Only Win32 Controller is supported.

...
4. **Successor Processing**  
   - If the action succeeds, enter the matched node's `next` list and repeat the above detection process
   - If the action fails, enter the matched node's `on_error` list
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    When the current node fails (recognition timeout or action execution failure), the nodes in this list are executed next. Optional, empty by default.  
    💡 Since v5.1, nodes with attributes, NodeAttr objects, or heterogeneous arrays combining both are supported. See [Node Attributes](#node-attributes) for details.

...
- `is_sub`: *bool* **(Deprecated in version 2.x, but retains compatibility.`interrupt` is recommended instead.)**\
...
- `timeout_next`: *string* | *list<string, >* **(Deprecated in version 2.x, but retains compatibility.`on_error` is recommended instead.)**\

...
- `labels`: *list<string, >*  
    Labels, meaning the names of each category. Optional.  
    Labels only affect debug images and logs. They are resolved from the Pipeline, ONNX metadata, and the same-name model JSON in that order. If none is available, MaaFramework generates `Unknown_<class_id>`. In general, this field does not need to be set; configure labels in the same-name model JSON or use ONNX metadata instead.

- `model`: *string*

    Model file path relative to the `model/detect` folder. Required.  
    Include the `.onnx` extension. Official Ultralytics models using the `Detect` head, plus RT-DETR exports, can be identified from metadata. Other models must use a same-name JSON descriptor to complete preprocessing and output semantics. An incomplete contract produces `ModelError`. See the [Neural-network object-detection model protocol](3.4-NeuralNetworkObjectDetectionModelProtocol.md).
    For Ultralytics export examples, see the [NNDetect Recipe](https://github.com/MaaXYZ/MaaNeuralNetworkCookbook/tree/main/NeuralNetworkDetect).

- `expected`: *int* | *list<int, >*  
    Expected category indices. Optional.  
    When non-empty, at most the highest-scoring result for each expected class is retained. When empty, all results that pass the threshold and NMS are retained.
...
- `multi_label`: *boolean*
    ...
    Whether every qualifying class is retained for each proposal/query that carries class scores. Optional, default is `true`.
    When `false`, the best class is selected for each proposal/query first. Outputs that already carry class IDs are unaffected.

- `input_size`: *array<int, 2>*
    Dynamic model input size in `[width, height]` order. Optional.
    It only fills dynamic ModelProto H/W dimensions; static dimensions always win. If omitted, resolution continues through metadata `imgsz`, model JSON `Resize.size`, and the current ROI size.
...
These values must match the actual model output. Configure NMS in the same-name model JSON; see the [neural-network object-detection model protocol](3.4-NeuralNetworkObjectDetectionModelProtocol.md).

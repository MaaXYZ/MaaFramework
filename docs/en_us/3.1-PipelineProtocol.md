...
> **Compatibility note:** The key names above are also compatible with the non-`$` prefix form (e.g., `Default`, `TemplateMatch`, `Click`), but both forms of the same key name cannot coexist — doing so will cause an error. The `$` prefix form is recommended.
...
2. Default parameters in the `$Default` object of the Pipeline JSON file (applies to current file only)
3. Default parameters for the corresponding algorithm/action type in `default_pipeline.json` (applies to current Bundle only)
4. Default parameters in the `$Default` object of `default_pipeline.json` (applies to current Bundle only)
5. Built-in default values in the framework

> The inheritance chain above applies to **nodes loaded from Pipeline files**. For nodes created dynamically via `MaaResourceOverridePipeline` / `MaaContextOverridePipeline`, the default baseline is the framework built-in defaults, and it does not inherit from `default_pipeline.json` or file-level `$Default`.
...
- For nodes created dynamically via `MaaResourceOverridePipeline` / `MaaContextOverridePipeline`, explicitly provide required values in override JSON (no automatic inheritance from `default_pipeline.json` or file-level `$Default`)

#### Multiple Bundle Loading Behavior

When loading multiple Bundles sequentially:

- Each Bundle's `default_pipeline.json` **only applies to nodes within that Bundle**, and does not affect nodes from other Bundles
- **Already-loaded nodes are NOT affected by subsequent Bundle's `default_pipeline.json`**
- Each node uses the defaults from its own Bundle at the time of first loading, and remains unchanged thereafter
...
{ "$Default": { "rate_limit": 100 } }   // Only applies to nodes within Bundle_Debug
...
#### File-Level Default Properties `$Default`

In a Pipeline JSON file, you can define a `$Default` node as **file-scoped** default properties. It works similarly to the `Default` object in `default_pipeline.json`, but its scope is limited to the current JSON file.
...
```jsonc
{
    ...
    "NodeA": {
        "recognition": "OCR"
        // rate_limit inherits 500 from $Default
        // pre_delay inherits 100 from $Default
    },
    "NodeB": {
        "recognition": "TemplateMatch",
        "template": "B.png",
        "rate_limit": 1000  // Overrides $Default value
    }
}
```
...
- `$Default` overrides the `Default` object from `default_pipeline.json`, then serves as the default for new nodes in the current file
- Only applies to **new nodes** in the current file. If a node was already defined in another file, it is not affected by `$Default` (existing data is used)
- `$Default` itself starts with `$` and will not be registered as a Pipeline node

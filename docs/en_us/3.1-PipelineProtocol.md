...
> JSON root fields that begin with a `$` will not be parse.
...
- `next` : *string* | *list<string, >*  
    ...
    💡 Node prefix functionality added in v5.1  

  - `[JumpBack]` Jump-back functionality  
...
- `is_sub`: *bool*  
    **(Deprecated in version 5.1, node prefix jump-back functionality `[JumpBack]` is recommended instead)**  
...
- `on_error` : *string* | *list<string, >*  
    ...
    💡 Supports the same node prefix functionality as `next` (`$` ignore, `[JumpBack]` jump-back).

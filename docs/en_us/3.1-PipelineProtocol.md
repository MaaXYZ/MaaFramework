...
- `anchor`: *string* | *list<string, >* | *object*  
    ...
    - **Object**: `"anchor": {"A": "TargetNode", "B": ""}` - Sets anchor A to TargetNode, clears anchor B (empty string means clear the anchor)
...
- `roi`: *array<int, 4>* | *string*  
  ...
  - *array<int, 4>*: Recognition area coordinates [x, y, w, h]. Supports negative values: negative x/y means calculating from right/bottom edge; w/h of 0 means extending to edge, negative means taking absolute value and treating (x, y) as bottom-right corner.
...
- `all_of`: *list\<string | object\>*  
    ...
    Each element can be:
  - string: node name reference, uses that node's recognition algorithm and parameters at runtime
...
- `target`: *true* | *string* | *array<int, 2>* | *array<int, 4>*  
  ...
  - *array<int, 2>*: Fixed coordinate point [x, y]. Supports negative values, meaning calculating from right/bottom edge.
  - *array<int, 4>*: Fixed coordinate area [x, y, w, h]. A point is sampled inside the rectangle with higher probability near center. Supports negative values, same meaning as `roi`.
...
Add a `focus` field to the node, where keys are message types and values are the content configuration to display.

Each message type value supports two forms:

- **String** (legacy form, backward compatible): Used directly as display content, equivalent to `display: "log"`.
- **Object** (new form): Specify content via `content` and display channel via `display`.

Template strings support file paths, URLs, or direct text. Content supports Markdown format and internationalization (starting with `$`). Placeholders in `{field_name}` format can be used in the template, and the UI will automatically replace them with actual values.

```jsonc
{
  "NodeA": {
    "focus": {
      // Legacy form: string, displayed in run log by default (backward compatible)
      "Node.Recognition.Succeeded": "{name} recognition hit, ready to start execution",

      // New form: object, specify display channel via display
      "Node.Action.Starting": {
        "content": "{name} starts execution, task ID: {task_id}",
        "display": "toast"
      },

      // display supports arrays, send to multiple channels simultaneously
      "Node.Action.Failed": {
...
| Value | Description |
| --- | --- |
| `"log"` | Default — appended to the run log |
| `"toast"` | **In-app** lightweight notification, briefly appears and auto-dismisses, non-blocking |
| `"notification"` | **System-level** notification, sent to the OS notification center/tray, suitable for notifying users when running in the background |
| `"modal"` | **Blocking** popup — task is paused until the user dismisses it |
| `"dialog"` | **Non-blocking** dialog — shows a message that the user can ignore while the task continues |

`display` can be a `string` or `array<string>`. Omitting it is equivalent to `"log"`.

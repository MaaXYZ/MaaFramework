...
The recognition phase is the starting point of task execution. Based on the configured algorithm and conditions, the system recognizes an image or specific pattern to determine whether to proceed with the next action.

- **Recognition Algorithm (`recognition`)**  
  Select the type of recognition algorithm, such as `DirectHit`, `TemplateMatch`, `OCR`, etc. (see [Algorithm Types](#AlgorithmTypes)). The default is `DirectHit`.
  
- **Invert Recognition Result (`inverse`)**  
  If `inverse` is set to true, recognized results are treated as not recognized, and unrecognized results are treated as recognized. This is typically used for reverse logic, with the default set to false.

- **Rate Limit (`rate_limit`)**  
  Limits the recognition process speed, specified in milliseconds. The default is 1000. Each recognition cycle takes at least the specified `rate_limit` milliseconds. If it takes less, the system will sleep to wait.

- **Recognition Timeout (`timeout`)**  
  Specifies the timeout for the recognition phase, in milliseconds. The default is 20,000 milliseconds. If no recognition occurs within the timeout period, `on_error` may be triggered.

- **Enabled Status (`enabled`)**  
  Determines whether the task is enabled. If `enabled` is set to false, this task will be skipped and neither recognized nor executed.

- **Notifications (`focus`)**  
  If `focus` is set to true, callback messages will be generated to monitor the status and progress of the task.

### **2. Execution Phase**

After a successful match in the recognition phase, the execution phase begins. In this phase, the specified action is carried out, handling the pre/post delays and the execution result.

- **Action (`action`)**  
  Specifies the type of action to be performed, such as `Click`, `Swipe`, `Text`, `Key`, etc. The default action is `DoNothing`. This is the core action of the task.

- **Pre-Action Delay (`pre_delay`)**  
  A delay before executing the action after successful recognition, specified in milliseconds. The default is 200 milliseconds.

- **Pre-Action Wait for Screen Freeze (`pre_wait_freezes`)**  
  Before executing the action, the system waits until the screen shows no significant changes for a specified amount of time.

- **Post-Action Delay (`post_delay`)**  
  A delay after executing the action, specified in milliseconds. The default is 200 milliseconds.

- **Post-Action Wait for Screen Freeze (`post_wait_freezes`)**  
  After the action is executed, the system waits until the screen becomes stable (no significant changes).

### **3. Follow-Up Tasks**

After task execution, the system decides whether to execute subsequent tasks based on the logic defined. If recognition or execution fails, it can also handle interrupts and errors.

- **Next Task (`next`)**  
  Defines a list of follow-up tasks. The system will recognize and execute the first matched task in the `next` list, in sequence.

- **Interrupt Task (`interrupt`)**  
  If no tasks in the `next` list are recognized, the system will attempt the tasks in the `interrupt` list, executing the first recognized task. After completing the interrupt task, the system will retry recognizing the `next` tasks.

- **Error Handling (`on_error`)**  
  If a timeout occurs or the task execution fails, the system will execute the tasks in the `on_error` list. This is an error-handling mechanism that ensures the task flow continues despite errors.

...
- `auto_up`: *bool* **💡 v5.10**  
    Whether to automatically lift still-held contacts when the task is stopped, finished, or the controller is destroyed. Optional, default `true`.  
    If the user stops the task midway, or the task ends / the controller is destroyed without a matching `TouchUp`, the contact is lifted automatically so the device is not left pressed.  
    A matching `TouchUp` will not be followed by another automatic lift. Set to `false` to keep the contact held after the task ends.
...
- `auto_up`: *bool* **💡 v5.10**  
    Whether to automatically release still-held keys when the task is stopped, finished, or the controller is destroyed. Optional, default `true`.  
    If the user stops the task midway, or the task ends / the controller is destroyed without a matching `KeyUp`, the key is released automatically so the device is not left pressed.  
    A matching `KeyUp` will not be followed by another automatic release. Set to `false` to keep the key held after the task ends.

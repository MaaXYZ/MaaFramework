...
| v5.10 | Time fields (`delay` / `timeout` / `duration` / `rate_limit`, etc.) support `[min, max]` random ranges; Added `auto_up` to `TouchDown` / `KeyDown`; automatically release still-held contacts/keys when the task is stopped, finished, or the controller is destroyed |

### Random time range **💡 v5.10**

The following time fields accept a single integer, or a `[min, max]` pair as inclusive bounds. At runtime a value is sampled uniformly from the closed interval:

- Node fields: `rate_limit`, `timeout`, `pre_delay`, `post_delay`, `repeat_delay`
- Numeric form of `pre_wait_freezes` / `post_wait_freezes` / `repeat_wait_freezes`, and `time` / `rate_limit` / `timeout` inside the object form
- `duration` of `LongPress` / `LongPressKey`
- `shell_timeout` of `Shell`
- `starting` of `MultiSwipe`
- `duration` / `end_hold` of `Swipe` / `MultiSwipe`: each element may be `uint` or `[min, max]`. For historical reasons `[a, b]` still means two swipe segments; use `[[a, b]]` for a single-segment random range

`get_node_data` / `get_node_object` return the original config (a single integer or a range). Sampling happens when sleeping or executing the action.  
Equal-bound ranges such as `[100, 100]` are equivalent to a scalar and are normalized to a scalar when dumped.
...
- `rate_limit`: *uint* | *array<uint, 2>*  
    ...
    If `[min, max]` is given, a value is sampled uniformly at runtime. See [Random time range](#random-time-range).

- `timeout`: *int* | *array<int, 2>*  
...
- `pre_wait_freezes`: *uint* | *array<uint, 2>* | *object*  
    ...
    If `[min, max]` is given, a value is sampled uniformly at runtime. See [Random time range](#random-time-range).  
    The specific order is `pre_wait_freezes` - `pre_delay` - `action` - `post_wait_freezes` - `post_delay`.

- `post_wait_freezes`: *uint* | *array<uint, 2>* | *object*  
...
- `repeat_delay`: *uint* | *array<uint, 2>* **💡 v5.3**  
...
- `repeat_wait_freezes`: *uint* | *array<uint, 2>* | *object* **💡 v5.3**  
...
- `expected`: *int* | *string* | *list\<int | string\>*  
    Expected classification index or category name. List items can be a mix of integers and strings. Optional, default matches all.
...
Both ways are equivalent and can be mixed (e.g., [0, "Mouse"]).
...
- `contact`: *uint* **💡 v5.0**  
  ...
  - Android Native Controller: Finger id (0 for first finger, valid range 0–15, matching Android MotionEvent pointer id)
...
- `duration`: *uint* | *array<uint, 2>* | *list<uint | array<uint, 2>,>*  
    Duration of the swipe in milliseconds. Optional, default is 200.  
    Each element may be a single value or a `[min, max]` random range. `[a, b]` means two swipe segments; use `[[a, b]]` for a single-segment random range. See [Random time range](#random-time-range).

- `end_hold`: *uint* | *array<uint, 2>* | *list<uint | array<uint, 2>,>*  
    After swiping to the end point, wait for an additional time before lifting, in ms. Optional, default 0.  
    Each element may be a single value or a `[min, max]` random range. `[a, b]` means two hold durations; use `[[a, b]]` for a single-segment random range. See [Random time range](#random-time-range).
...
- `auto_up`: *bool* **💡 v5.10**  
    Whether to automatically lift still-held contacts when the task is stopped, finished, or the controller is destroyed. Optional, default `false`.  
    If set to `true` and the user stops the task midway, or the task ends / the controller is destroyed without a matching `TouchUp`, the contact is lifted automatically so the device is not left pressed.  
    A matching `TouchUp` will not be followed by another automatic lift.
...
- `auto_up`: *bool* **💡 v5.10**  
    Whether to automatically release still-held keys when the task is stopped, finished, or the controller is destroyed. Optional, default `false`.  
    If set to `true` and the user stops the task midway, or the task ends / the controller is destroyed without a matching `KeyUp`, the key is released automatically so the device is not left pressed.  
    A matching `KeyUp` will not be followed by another automatic release.
...
- `shell_timeout`: *int* | *array<int, 2>* **💡 v5.8**  

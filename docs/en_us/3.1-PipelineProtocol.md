...
When loading multiple Bundles sequentially, their `default_pipeline.json` files are merged (dict merge) in order. Later-loaded fields with the same name override earlier ones. Merging is based on top-level keys (`Default`, algorithm names, action names) independently.

**Example:** `Bundle_Base` → `Bundle_Special`

```jsonc
// Bundle_Base/default_pipeline.json
{
    "Default": { "rate_limit": 2000, "timeout": 30000 },
    "TemplateMatch": { "threshold": 0.7 }
}

// Bundle_Special/default_pipeline.json
{
    "Default": { "rate_limit": 1000 },  // Overrides, timeout retained
    "OCR": { "threshold": 0.5 }
}

// Final result
{
    "Default": { "rate_limit": 1000, "timeout": 30000 },
    "TemplateMatch": { "threshold": 0.7 },
    "OCR": { "threshold": 0.5 }
}
```
...
**Example:** Separate debug nodes into an independent Bundle with shorter delays and lower thresholds

```cpp
MaaResourcePostPath(resource, "resource/base");   // rate_limit: 2000
MaaResourcePostPath(resource, "resource/debug");  // rate_limit: 100, threshold: 0.5
```

The `debug` Bundle's defaults override same-named fields from `base` Bundle. To prevent `base` nodes from being affected, explicitly set parameters in nodes (highest priority).

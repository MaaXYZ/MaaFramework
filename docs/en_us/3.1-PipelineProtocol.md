...
- `roi`: *array<int, 4>* | *string*  
  ...
  - *string*: Fill in the node name, and identify within the target range identified by a previously executed node. Also supports `[Anchor]AnchorName` format to reference the node corresponding to an anchor **💡 v5.9**.
...
- `target`: *true* | *string* | *array<int, 2>* | *array<int, 4>*  
  ...
  - *string*: Enter the node name, as the target, to use the position recognized by a previously executed node. Also supports `[Anchor]AnchorName` format to reference the node corresponding to an anchor **💡 v5.9**.

...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    Note: No jump-back is performed while in error-handling state; `JumpBack` may happen only after a later hit restores non-error state and `next` becomes empty.  
    💡 Since v5.1, nodes with attributes, NodeAttr objects, or heterogeneous arrays combining both are supported. See [Node Attributes](#node-attributes) for details.
...
The lifecycle of a node is as follows (excluding cross-node controls such as `anchor` / `[JumpBack]`, and omitting the action-failure branch):
...
> [!NOTE]
>
> This diagram only covers the "single-node successful-action path + `next` recognition retry" main loop; for the action-failure path into `on_error`, see [Execution Logic](#execution-logic).  
> For cross-node controls such as `anchor` / `[JumpBack]`, see [Node Attributes](#node-attributes).  
> `need_to_stop` is checked before and after each action execution, so setting it during runtime may end the current node in the action phase.

...
- `anchor`: *string* | *list<string, >* | *object*  
    ...
    - **Object**: `"anchor": {"A": "TargetNode", "B": ""}` - Sets anchor A to TargetNode, anchor B to the current node (empty string means current node)
...
Using the object format, you can set an anchor to a different node instead of the current node:
...
```jsonc
{
    "A": {
        "anchor": {
            ...
            "Y": "",         // Anchor Y -> current node A (empty string means current node)
...
- Advanced usage: Allows mapping anchors to specific target nodes, providing more flexible flow control capabilities, similar to introducing a variable mechanism to the pipeline.

...
```jsonc
{
    "NodeA": {                   // Node name, unique identifier, globally unique
...
| v5.10 | Added `auto_up` to `TouchDown` / `KeyDown`; automatically release still-held contacts/keys when the task is stopped, finished, or the controller is destroyed |
...
- `timeout`: *int*  
    ...
    *A **special case** is that if the node is the first node of the `Task`, i.e., the `Entry` node, the `timeout` property also applies to its own `recognition` wait time.*  
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    *A **special case** is that if the node is the first node of the `Task`, i.e., the `Entry` node, it will enter its own `on_error` list after its own recognition fails.*  
    💡 Since v5.1, nodes with attributes, NodeAttr objects, or heterogeneous arrays combining both are supported. See [Node Attributes](#node-attributes) for details.

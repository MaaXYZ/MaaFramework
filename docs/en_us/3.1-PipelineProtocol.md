...
- `checkpoint` : *string* | *list<string, >*  
    Mark the current node as checkpoints with specified names. Optional, empty by default.  
    Checkpoints are used with the `jumpback` field to implement jumping. When a node is successfully recognized and executed, if its `checkpoint` field is not empty, the node will be associated with the checkpoint names, allowing subsequent nodes to jump back via `jumpback`.  
    For example:  
    ...
    When task entry is A1 and A2 respectively, the pipeline will be:  
        - `A1` -> `B` -> `C` -> `A1` -> ...
        - `A2` -> `B` -> `C` -> `A2` -> ...
    This allows reuse of nodes B and C.  

    Additionally, a single node can be associated with multiple `checkpoint` names (array); different nodes can use the same `checkpoint` name, and later executions will override previous ones; note that checkpoints are task-level and not shared between different tasks.

- `jumpback` : *string* | *list<string, >*  
    After successfully executing the current node, jump to nodes corresponding to the specified checkpoint names with priority. Optional, empty by default.  
    Consistent with the `next` logic, it recognizes each node in the `jumpback` list in order and executes only the first recognized one.  
    If a checkpoint is not recorded, it will be ignored. Note that checkpoints are task-level and not shared between different tasks.  
    When `jumpback` is not empty, `next` and `interrupt` will not take effect.

- `interrupt` : *string* | *list<string, >*  
    **(Deprecated in version 5.x, but retains compatibility. `checkpoint` + `jumpback` is recommended instead.)**  
...
- `is_sub`: *bool*  
    **(Deprecated in version 2.x, but retains compatibility. `checkpoint` + `jumpback` is recommended instead.)**  
...
- `timeout`: *uint*  
    ...
    The detailed logic is `while(!timeout) { foreach(jumpback OR (next + interrupt)); sleep_until(rate_limit); }`  
    Note: When jumpback is not empty, it takes priority; otherwise, next + interrupt is used.

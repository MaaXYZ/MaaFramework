...
- `labels`: *list<string, >*  
    Labels, meaning the names of each category. Optional.  
    Labels only affect debug images and logs. They are resolved from the Pipeline, the same-name model JSON, and ONNX metadata in that order. If none is available, MaaFramework generates `Unknown_<class_id>`. In general, this field does not need to be set; configure labels in the same-name model JSON or use ONNX metadata instead.

- `model`: *string*  
    Model file path relative to the `model/detect` folder. Required.  
    Include the `.onnx` extension. Input and output tensor protocol combinations support the Maa YOLO Dense compatibility format, YOLOv8, YOLO11, YOLO26, RT-DETR, RF-DETR, D-FINE, and LW-DETR. Only the legacy Maa YOLO Dense compatibility format may omit the same-name model descriptor. See the [Neural-network object-detection model protocol](3.4-NeuralNetworkObjectDetectionModelProtocol.md).  
    For YOLOv8 and YOLO11 training, see the [NNDetect Recipe](https://github.com/MaaXYZ/MaaNeuralNetworkCookbook/tree/main/NeuralNetworkDetect).
...
- `nms`: *string*  
    Non-maximum suppression policy. Optional.  
    Possible values: `None` | `ClassAwareIoU` | `CandidateCoverage`.  
    Configuration priority is Pipeline, same-name model JSON, then the output-protocol default. In general, this field does not need to be set; configure it in the same-name model JSON instead.

- `nms_threshold`: *double*  
    NMS threshold. Optional, in the range [0, 1].  
    When specified alone, it overrides the threshold of the currently effective NMS policy. It is ignored when the policy is `None`. In general, this field does not need to be set for the same reason described above.

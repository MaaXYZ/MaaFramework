...
- `next_override`: *object*  
    Override configuration mapping table for nodes in `next`. Optional, default is empty.  
    When executing nodes in the `next` list, if a node name exists in `next_override`, the override configuration will be used to temporarily modify that node's configuration, while the node's own configuration remains unchanged.  

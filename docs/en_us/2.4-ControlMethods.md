...
### Gamescope Prerequisites

1. Linux only
2. `libpipewire >= 0.3.50` and `libei >= 1.0` must be installed
3. Must run within a Gamescope environment

### Obtaining node_id

`node_id` is the PipeWire node ID of the Gamescope output stream. It can be obtained as follows:

```bash
# Find using pw-dump and jq
...
### Obtaining eis_socket_path

`eis_socket_path` is the EIS socket path for libei input emulation. It is typically located under `$XDG_RUNTIME_DIR`:

```bash
# Check gamescope EIS sockets
ls $XDG_RUNTIME_DIR/gamescope-*-ei
# Path determined by gamescope environment variables, typically under $XDG_RUNTIME_DIR
```
...
- screencap: Captures screenshots from the Gamescope output stream via PipeWire `pw_stream`
- click / swipe / touch_down / touch_move / touch_up: Emulates absolute pointer and button events via libei
- click_key / key_down / key_up: Emulates keyboard events via libei
- input_text: Supports ASCII printable characters (translated to evdev keyboard events internally)
- scroll: Emulates scroll wheel events via libei
- relative_move: Emulates relative pointer motion via libei

### Unsupported operations

- start_app: Not supported (returns false)
- stop_app: Not supported (returns false)
...
# Obtain node_id via pw-dump
# eis_socket_path is determined by gamescope environment variables

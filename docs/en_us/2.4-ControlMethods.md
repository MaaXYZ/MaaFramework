...
> - When the target window is minimized, `AnchoredTouch` shows it at the lowest opacity before resolving the coordinates, and minimizes it again once all contacts are released; if the user brings that window to the foreground meanwhile, it is left alone. This covers only windows that `AnchoredTouch` itself took out of the minimized state. The pseudo-minimize of the `FramePool` / `PrintWindow` screencap methods keeps its own state and is restored by that side, which on `inactive` restores the style and opacity without minimizing the window again.

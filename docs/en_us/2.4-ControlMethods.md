...
> - Win32 also provides a **Mouse Lock Follow** mode: enable with `MaaControllerPostMouseLockFollow(ctrl, true)`. Designed for TPS/FPS games that lock the mouse to the window in the background. When enabled, the window continuously follows the mouse cursor, and RawInput counter-moves prevent the game from sensing hardware mouse movement. Use `MaaControllerPostRelativeMove` to inject intentional camera rotation while this mode is active. Only supported with MessageInput-based input methods (SendMessage / PostMessage variants).

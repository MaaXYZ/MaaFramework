...
> - Win32 also provides a **Background Managed Keys** guardian: declare the managed key domain through `MaaControllerSetOption(ctrl, MaaCtrlOption_BackgroundManagedKeys, keycodes, sizeof(int32_t) * count)` with an array of virtual key codes to manage. Must be set before connection. After setting, matching key operations automatically route through the background guardian path and continuously correct key state while the controller is idle. This feature has extremely low compatibility and is currently validated only for Endfield background WASD.

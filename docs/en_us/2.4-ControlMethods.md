...
- **Monitor capture** (default): pass the compositor's PipeWire FD (`pw_socket_fd`) and node ID (`pw_node_id`)
...
- **Session-daemon node capture**: no FD; set `pw_node_id` to attach to a node on the session PipeWire daemon
  (e.g. gamescope's Video/Source node). Discover the node ID with `MaaToolkitGamescopeNodeFindAll`.

When using `Libei` input method, `eis_socket_path` must point to the EIS socket provided by the compositor, e.g.
gamescope's `/run/user/<uid>/gamescope-<n>-ei`.
...
The Gamescope controller controls applications running in a
[Gamescope](https://github.com/ValveSoftware/gamescope) microcompositor on Linux (e.g. SteamOS game mode or a nested
gamescope session). Configure `MaaLinuxControllerCreate`'s `config_json` as follows:

- `screencap_method`: `PipeWire` (session-daemon node capture)
- `pw_node_id`: the gamescope PipeWire node ID; discover it with `MaaToolkitGamescopeNodeFindAll` (each gamescope
  instance publishes one `Video/Source` node). The captured image is the gamescope output (the game's content
  resolution, e.g. 1280x720)
- `input_method`: `Libei`
- `eis_socket_path`: the EIS socket path provided by gamescope, e.g. `/run/user/<uid>/gamescope-<n>-ei`
- `use_win32_vk_code`: optional, keycode interpretation, see Keyboard Input above

...
> - Win32 also provides a background key guardian independent from `keyboard_method`: after declaring the managed key domain through `MaaControllerPostSetBackgroundManagedKeys`, matching `ClickKey` / `LongPressKey` / `KeyDown` / `KeyUp` operations automatically route through the guardian path and keep correcting key state while the controller is idle.
> - This background key guardian has extremely low compatibility and is currently validated only for Endfield background WASD. It sends an `F13` nudge on managed key press, and releases all managed keys on `MaaControllerPostInactive`, stop, or natural queue drain.
> - The background key guardian registers global hotkeys with IDs in the range 2000–2255 (`kManagedHotkeyBase + keycode`); applications using overlapping hotkey IDs may conflict.
...
Keycodes for MaaControllerPostKey{Down,Up} in WlRoots controller is evdev key codes, defined in [linux/input-event-codes.h](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/input-event-codes.h).

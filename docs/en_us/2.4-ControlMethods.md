...
| AutoForeground | `1 << 48` | N/A | N/A | No | No | Try `DXGI_DesktopDup_Window -> GDI -> ScreenDC` in order |
| AutoBackground | `1 << 49` | N/A | N/A | No | Yes | Try `FramePool -> PrintWindow` in order |

> [!NOTE]
>
> FramePool and PrintWindow have built-in pseudo-minimize support: when the target window is minimized, they make it transparent and click-through, then restore it without activation, allowing screencap to continue without disturbing the user. `AutoBackground` inherits this behavior because it tries these two methods in order.
> Other screencap methods (including `AutoForeground`) will fail when the target window is minimized. Please avoid minimizing the window when using those methods.

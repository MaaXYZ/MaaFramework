...
The Android native controller supports multi-touch and Pipeline `MultiSwipe`. `touch_down` and `touch_move` pass `contact` and `pressure` to the external control unit, while `touch_up` passes `contact`.
...
When using `PipeWire` screencap method, the PipeWire FD and node ID obtained from `xdg-desktop-portal` must be passed
in.

...
If you prefer using Win32 Virtual-Key codes (VK_*), enable it with `MaaControllerSetOption(ctrl, MaaCtrlOption_WlRootsUseWin32VkCode, &enabled, sizeof(bool))` (set `enabled` to `true` to enable, `false` to disable). When enabled, key codes passed to `MaaControllerPostClickKey` / `MaaControllerPostKey{Down,Up}` are interpreted as Win32 VK codes and translated to evdev codes internally. This option is only valid for WlRoots controllers and makes it easy to reuse key handling logic written for the Win32 controller.

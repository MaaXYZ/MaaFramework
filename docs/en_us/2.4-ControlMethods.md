...
The KWin controller is used in the Linux Wayland (KWin) desktop environment to implement screen capturing and input injection via PipeWire and `/dev/uinput`. It is still called the "KWin controller" because it was originally developed and tested primarily on the KWin compositor, and the name has been retained since.
...
2. **Permissions Required:** The current user must have read and write permissions for the `/dev/uinput` device.
   - **Recommended udev Rule Configuration**: Create `/etc/udev/rules.d/99-uinput.rules` and add the following content: `KERNEL=="uinput", MODE="0660", GROUP="input"`. Then, add the current user to the `input` group and reboot for the changes to take effect.

> [!TIP]
> 
> The KWin controller can be used with any system whose desktop environment implements the XDG Screencast Portal and whose kernel has the uinput module enabled (e.g., GNOME).

### KWin Screencap

The KWin controller utilizes PipeWire in conjunction with the `xdg-desktop-portal` protocol for screen capturing.

> [!NOTE]
>
> - **Authorization Interaction**: When the screen capturing feature is invoked for the first time, the desktop environment will trigger a native screencast authorization dialog, requiring the user to manually click "Allow".

### KWin Input

The KWin controller handles input simulation via a kernel-level virtual input device.

### Keyboard input notice

Keycodes for MaaControllerPostKey{Down,Up} in the KWin controller are **by default** evdev key codes, defined in [linux/input-event-codes.h](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/input-event-codes.h). This behavior is consistent with the [WlRoots controller](#wlroots-linux).

If you prefer using Win32 Virtual-Key codes (VK_*), pass `true` for the `use_win32_vk_code` parameter when calling `MaaKWinControllerCreate`. When enabled, key codes passed to `MaaControllerPostClickKey` / `MaaControllerPostKey{Down,Up}` are interpreted as Win32 VK codes and translated to evdev codes internally. This flag is decided at creation time and cannot be changed at runtime; it makes it easy to reuse key handling logic written for the Win32 controller.

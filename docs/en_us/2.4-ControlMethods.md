...
| AnchoredTouch | `1024` | Medium | No | No | Yes | Injects synthetic touch points, the target window receives `WM_POINTER` messages, mouse only |
...
> - `AnchoredTouch` injects synthetic touch points through `InjectSyntheticPointerInput`, requires Windows 10 1809 or later. It never moves the cursor and never changes the foreground window, so it does not disturb the user, and applications that render according to the cursor position are not affected. Because it only implements mouse operations, select another method for keyboard input.

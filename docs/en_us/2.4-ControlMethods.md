...
> [!NOTE]
>
> - **Authorization Interaction**: When the screen capturing feature is invoked for the first time, the KDE desktop will trigger a native ScreenCast authorization dialog, requiring the user to manually click "Allow".
...
> [!NOTE]
> 
> - **Keyboard Input**: Full keyboard emulation is supported. By default, keycodes dispatched by MAA are mapped and sent as `KEY_*` macro definitions from `<linux/input-event-codes.h>`. In the event of a conflict between a Linux keycode and an ASCII character, priority will be given to ensuring the accurate mapping of the ASCII character.
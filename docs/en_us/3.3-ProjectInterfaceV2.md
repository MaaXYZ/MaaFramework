...
* **SendMessageWithCursorPosAndBlockInput** `128`
  Based on `SendMessageWithCursorPos`, adds `BlockInput`; blocks user input during operation to prevent interference. Requires Administrator privileges.

* **PostMessageWithCursorPosAndBlockInput** `256`
  Based on `PostMessageWithCursorPos`, adds `BlockInput`; blocks user input during operation to prevent interference. Requires Administrator privileges.
...
| Input | SendMessageWithCursorPos | Yes | - | - | If target is elevated | Synchronous messages + cursor movement, for games detecting mouse position |
| Input | PostMessageWithCursorPos | Yes | - | - | If target is elevated | Asynchronous messages + cursor movement, for games detecting mouse position |
| Input | SendMessageWithCursorPosAndBlockInput | Yes | - | - | Administrator required | Synchronous messages + cursor movement + block input, prevents interference |
| Input | PostMessageWithCursorPosAndBlockInput | Yes | - | - | Administrator required | Asynchronous messages + cursor movement + block input, prevents interference |

...
| 2026-05-07 | v2.7.1  | `pretask` may also be defined in files referenced by `import`; merged in the same order as `task` / `preset` (main file first, then each imported file in `import` order) |
...
- **pretask** `object | object[]` **💡 v2.7.0**  
  ...
  **💡 v2.7.1** The same `pretask` field may appear in files loaded through the top-level `import` array; the Client should merge all resulting pre-tasks into one ordered list: first those from the main `interface.json` (a single object counts as one entry), then those from each imported file in the order listed in `import`, appending each file's own `pretask` entries in array order (or the single object, if not an array). Execution order follows that merged list.
...
- **import** `string[]` **💡 v2.2.0**  
  ...
  Supports importing `task`, `option`, `preset` **💡 v2.3.0**, and `pretask` **💡 v2.7.1** fields from these files.  

...
- **task** `object[]`  
  ...
  - **controller** `string[]`  
    ...
    When a user selects a controller, only tasks that support that controller will be displayed in the user interface for selection. This allows providing specialized task configurations for different controller types, such as tasks that are only available for Adb controllers or Win32 controllers.
...
- **option** `record<string, object>`  
  ...
  - **default_case** `string`  
    Default option name, used only when `type` is `"select"` or `"switch"`. The Client can use this value as the initial selected value for the option. _Optional._
...
- **import** `string[]` **💡 v2.2.0**  
  ...
  Only `task` and `option` fields from these files will be imported.  

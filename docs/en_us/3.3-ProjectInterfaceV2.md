...
| 2026-08-19 | v2.10.0 | Added string arrays to `welcome` |
...
- **welcome** `string | string[]` **💡 v2.10.0**  
  Welcome message displayed in a popup when the user first uses the application; it can also be used as an announcement. Each string supports file paths, URLs, or direct text. Content supports Markdown and internationalization (starting with `$`). Announcement titles should be part of the Markdown content.  
  Starting with v2.10.0, a string array can declare multiple announcements, which the Client must display in array order. The array must not be empty.  
  ...
  The Client must track the complete ordered announcement list. Adding, removing, or reordering announcements, or changing any content, counts as an update and should prompt the user again. Older Clients may not parse the array form; continue using a single string when compatibility with those Clients is required.

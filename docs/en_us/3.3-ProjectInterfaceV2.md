...
- **import** `string[]` **💡 v2.2.0**  
  ...
  Supports importing `task`, `option`, and `preset` **💡 v2.3.0** fields from these files. Imported files do not support declaring top-level `group`; imported tasks may still use `task.group` to reference groups defined in the main `interface.json`.  

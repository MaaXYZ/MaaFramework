...
* **task** `object[]`
  ...
  * **pipeline_override** `pipeline`
    *Optional.* Task parameters that [override](#resource-override) the loaded resources when executing the task. This structure is exactly the same as the JSON file in the `pipeline` and must include the task name part, for example:
...
* **option** `record<string, object>`
  ...
  * **default_case** `string`
    Default option name, used only when `type` is `"select"`. The Client can use this value as the initial selected value for the option. *Optional.*

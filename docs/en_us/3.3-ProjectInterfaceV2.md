...
* **Seize**
  ...
  Exclusive control. Uses `SendInput` with foreground focus; steals focus, moves the system cursor, and blocks user input during operations. Not suitable for background control.
...
* **LegacyEvent**
  ...
  Legacy `mouse_event` / `keybd_event`; depends on foreground and system cursor, blocks user input during operations, generally poorer compatibility. Not suitable for background control.
...
| Input | Seize | No (needs foreground) | - | - | Administrator required | Steals focus, moves system cursor, blocks user input |
...
| Input | LegacyEvent | No (needs foreground) | - | - | Administrator required | Legacy API, weaker compatibility, blocks user input |

...
| 2026-08-19 | v2.10.0 | Added announcement object arrays to `welcome` |
...
- **interface_version** `number`  
  The interface version number, currently 2, fixed and required. Identifies the **major JSON schema version** of `interface.json`. The PI documentation also uses an independent semantic version (e.g. v2.5.0 for the environment-variable convention); the two are not required to match.
...
- **welcome** `string | object[]` **💡 v2.10.0**  
  Welcome message displayed in a popup when the user first uses the application; it can also be used as an announcement. The string form supports file paths, URLs, or direct text. Content supports Markdown and internationalization (starting with `$`).  
  Starting with v2.10.0, an object array can declare multiple announcements, which the Client must display in array order. The array must not be empty. Each object contains the following fields:  
  - **label** `string`  
    Announcement title. _Optional_; supports internationalization (starting with `$`). No title is displayed when omitted.  
  - **content** `string`  
    Announcement content. Required; supports file paths, URLs, or direct text. Content supports Markdown and internationalization (starting with `$`).  
  ...
  "welcome": [
      ...
      {
          "content": "announcements/2026/update.md"
      }
  ]
  ```

  The Client must track the complete ordered announcement list. Adding, removing, or reordering announcements, or changing any `label` or `content`, counts as an update and should prompt the user again. Older Clients may not parse the array form; continue using the string form when compatibility with those Clients is required.
...
- **telemetry** `object` **💡 v2.9.0**  
  Anonymous telemetry (data reporting) configuration, used to report crashes and task run statistics to the resource author's own telemetry platform. May not be supported by all Clients. _Optional_; when not configured, no telemetry is performed. This field serves as a unified container for telemetry configuration; it currently provides the `sentry` sub-field and may be extended with other platforms in the future.

  Data ownership and privacy conventions:  
  - The telemetry configuration points to the resource author's own platform project, so data is naturally isolated per project; different software using different configurations can be distinguished. If multiple software share the same configuration, the Client should distinguish them within the same project via `release` / tags (e.g. project name, `version`).  
  - The Client must respect user consent first: even if this field is configured, whether to report is still decided by a user-facing toggle in the Client (recommended default on, can be turned off anytime, and force-disabled in debug / development builds).  

  - **sentry** `object`  
    Telemetry configuration based on [Sentry](https://sentry.io).

    - **dsn** `string`  
      The DSN of the Sentry project. Required; Sentry telemetry is disabled when it is missing or an empty string.

    - **tracing** `boolean`  
      Whether to enable performance / transaction reporting (task start, end, and each subtask result are reported as transactions / spans). _Optional_, defaults to `true`. When disabled, only crashes / errors are reported, not the task lifecycle.
...
- **controller** `object[]`  
  ...
  - **linux** `object`  
    Specific configuration for the `Linux` controller. See [Control Methods](2.4-ControlMethods.md#linux) for details.

    - **screencap** `string`  
      _Optional._ The screenshot method for the Linux controller. If not provided, the default is used.

    - **input** `string`  
      _Optional._ The input control method for the Linux controller. If not provided, the default is used.

    - **use_win32_vk_code** `bool`
      _Optional._ When `true`, key codes are interpreted as Win32 Virtual-Key codes and translated to Linux evdev codes internally; when `false`, they are passed through as raw evdev codes. Default is `false`.

    - **pipewire_source** `string`  
      _Optional._ The PipeWire screencap source: `Gamescope` connects to the gamescope node directly (node id discovered at runtime), `Portal` uses xdg-desktop-portal ScreenCast. Only valid when `screencap` is `PipeWire`. Default is `Gamescope`.
...
### Agent subprocess environment variables **💡 v2.5.0**

When the Agent runs in a separate process from the MaaFramework host, the child process has no automatic access to the general UI’s name and version, UI language, or which controller and resource package the user selected—information that is also not exposed through ordinary MaaFW C API callbacks. To let custom recognition/action logic read **Client-side context** and a **runtime snapshot of the current PI selection**, the **Client** should set the following environment variables when starting the `agent` subprocess (names in uppercase; values are strings).

| Variable | Description |
| -------- | ----------- |
| `PI_INTERFACE_VERSION` | Semantic version string of the PI **extension surface** implemented by the Client (e.g. `v2.5.0`). The child may use this to detect available variables or behaviors; **do not** confuse this with the numeric `interface_version` field in `interface.json` (currently fixed at `2`). |
| `PI_CLIENT_NAME` | Identifier of the general UI / Client, e.g. `MFAA`, `MXU`, `VSCODE`, `MPE`, `MaaDebugger`. |
| `PI_CLIENT_VERSION` | The Client’s own version (semantic version string as defined by the Client). |
| `PI_CLIENT_LANGUAGE` | Current UI language code, e.g. `zh_cn`, `en_us` (aligned with `languages` keys or Client convention). |
| `PI_CLIENT_MAAFW_VERSION` | MaaFramework library version bundled or used by the Client (semantic version string). |
| `PI_VERSION` | Same as the top-level `version` field in `interface.json` (resource project version). |
| `PI_CONTROLLER` | **Single-line JSON string**: the full object for the **currently selected** `controller` array entry. All i18n-capable fields (e.g. `label`, `description`) should already be resolved to **final display text**, without `$`-prefixed translation keys. |
| `PI_RESOURCE` | **Single-line JSON string**: the full object for the **currently selected** `resource` array entry, with i18n resolved likewise; use compact JSON (no newlines) so it fits environment variable limits. |

**Notes:**

- If a value is unavailable, the Client may omit the variable or set it empty; the child should tolerate missing variables.
- `PI_CONTROLLER` / `PI_RESOURCE` describe the **current selection** only. If the user switches controller or resource at runtime, the Client should update the environment when restarting the subprocess or at another agreed time (restart policy is implementation-defined).
- Values must respect OS limits on environment variable size and character set; JSON should be minified.

**Example (illustrative; actual values are single lines):**
...
- **option** `record<string, object>`  
  ...
  - **type** `string`  
    ...
    - `"hotkey"`: Hotkey capture field, allows the user to capture a shortcut by pressing keys **💡 v2.8.0**  
  ...
  - **hotkeys** `object[]`  
    ...
    - **label** `string`  
      Hotkey field display name, shown in the user interface. Supports internationalized strings (starting with `$`). If not set, the value of the `name` field is displayed. _Optional._
...
- **setting** `object[]` **💡 v2.8.0**  
  _Optional._ Task setting UI declarations used by Clients such as MXU to render a dedicated setting area for global task configuration. The Client should treat this field as presentation metadata only and must not assume it affects MaaFramework runtime behavior directly.  
  Each item describes one setting section; the Client may render sections in the order they appear in the array.

  - **name** `string`  
    Unique section identifier, used as the section anchor and internal key.

  - **label** `string`  
    Section display name shown in the user interface. Supports internationalized strings (starting with `$`). If not set, the value of `name` is displayed. _Optional._

  - **description** `string`  
    Section description text. Supports file paths, URLs, or direct text. Content supports Markdown format. _Optional._ Supports internationalization (starting with `$`).

  - **icon** `string`  
    Section icon file path, relative to the project root directory. _Optional._ Supports internationalization (starting with `$`).

  - **option** `string[]`  
    _Optional._ An ordered list of option keys from the top-level `option` configuration. The Client should render these options in the given order and use the existing `option` protocol definitions for the actual controls.

  - **default_expand** `boolean`  
    _Optional_, defaults to `true`. Whether this section is expanded by default in the Client.
  ...
  "setting": [
      {
          "name": "task_global",
          "label": "$TaskGlobal",
          "description": "$TaskGlobalDesc",
          "icon": "setting/task_global.png",
          "option": ["BattleMode", "AutoDodge"],
          "default_expand": true
      }
  ]
  ```
...
- **import** `string[]` **💡 v2.2.0**  
  ...
  **Merge rules:**
  ...
  | --- | --- |
  | `task` | Appended to the end of the main file's `task` array |
  | `option` | Object merge; later imported files override same keys |
  | `global_option` | Appended to the end of the main file's array; deduplicated by option key, keeping the first occurrence **💡 v2.8.0** |
  | `setting` | Appended to the end of the main file's `setting` array **💡 v2.8.0** |
  | `preset` | Appended to the end of the main file's `preset` array |
  | `group` | Appended to the end of the main file's `group` array; deduplicated by `name`, keeping the first occurrence |

...
| 2026-07-22 | v2.9.0  | Added the `sentry` anonymous telemetry (data reporting) configuration field |
...
- **sentry** `object` **💡 v2.9.0**  
  Anonymous telemetry (data reporting) configuration, used to report crashes and task run statistics to the resource author's own [Sentry](https://sentry.io) project. May not be supported by all Clients. _Optional_; when not configured, no telemetry is performed.

  Data ownership and privacy conventions:  
  - The DSN points to the resource author's own Sentry project, so data is naturally isolated per project; different software using different DSNs can be distinguished. If multiple software share the same DSN, the Client should distinguish them within the same project via `release` / tags (e.g. project name, `version`).  
  ...
  - **dsn** `string`  
    The DSN of the Sentry project. Required; telemetry is disabled when it is missing or an empty string.

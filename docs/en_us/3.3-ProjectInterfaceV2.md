...
- **controller** `object[]`  
  ...
  - **type** `'Adb' | 'Win32' | 'PlayCover' | 'Gamepad' | 'WlRoots'`  
    Controller type; valid values are `Adb`, `Win32` (Windows only), `PlayCover` (macOS only), `Gamepad` (Windows only),
...
- **group** `object[]` **💡 v2.4.0**  
  ...
  - **order** `number`  
    _Optional._ Group display order. Smaller values are shown first; if not specified, the Client may use the definition order in the `group` array.
...
- **import** `string[]` **💡 v2.2.0**  
  ...
  Supports importing `task`, `option`, `group` **💡 v2.4.0**, and `preset` **💡 v2.3.0** fields from these files.  

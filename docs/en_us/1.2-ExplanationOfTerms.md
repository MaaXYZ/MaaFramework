...
![architecture](/docs/static/architecture-en.svg)
...
- **UI**  
    Short for User Interface, referring to the visual layer where users interact with the program.  
    In the MaaFW project, the UI builds configuration items from `interface.json`, then passes resource files to the MaaFW core as needed.
...
- **OCR**  
    Process and recognize images containing text, and extract the contained text and layout information.  

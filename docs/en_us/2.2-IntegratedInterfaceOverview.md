...
Connect. If startup, protocol validation, or custom registration fails, the client rolls back registrations made by the current attempt and tries to shut down the remote session. The listening socket is kept; if the PAIR was occupied, it is recreated on the next `connect`.

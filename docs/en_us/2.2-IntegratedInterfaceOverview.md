...
> Multi-touch is supported: `contact` is the finger id (0 for the first finger, valid range 0–15). The external library's `TouchArgs` must carry `contact`.
...
- `read_path`: Path to a directory of images (or a single image file). Images are loaded on connect and cycled on each screencap. All input operations are no-ops that return success.

Create debug controller

### MaaReplayControllerCreate

- `recording_path`: Path to the recording JSONL file written by `MaaRecordControllerCreate`. Screenshot paths are resolved relative to this file's parent directory.

Create replay controller

### MaaRecordControllerCreate

- `inner`: Inner controller to forward all operations to. The record controller does not take ownership of `inner`.
- `recording_path`: Path to the recording JSONL file to write. Screenshots are saved to a `{stem}-Screenshot` folder in the same directory. Replay with `MaaReplayControllerCreate` using the recorded file.

Create record controller that wraps an existing controller and records all operations
...
- `config_json`: Linux controller config JSON.
  - Required fields:
    - `screencap_method`: screencap method to use, see [Control Methods](2.4-ControlMethods.md#linux-screencap).
    - `input_method`: input method to use, see [Control Methods](2.4-ControlMethods.md#linux-input).
  - Wlroots Required fields:
    - `wlr_socket_path`: wayland socket path (e.g., "/run/user/1000/wayland-0").
  - PipeWire monitor capture (via xdg-desktop-portal) Required fields:
    - `pw_socket_fd`: The PipeWire socket FD (from the ScreenCast portal).
    - `pw_node_id`: The PipeWire Node ID.
  - PipeWire session-daemon node capture (gamescope etc.) Required fields:
    - `pw_node_id`: attach to a session-daemon node directly; discover the node with
      `MaaToolkitGamescopeInstanceFindAll`.
  - UInput Optional fields:
    - `uinput_path`: The uinput device node path, default is "/dev/uinput".
    - `uinput_screen_width`: The screen width in pixels for the uinput absolute axis range.
    - `uinput_screen_height`: The screen height in pixels for the uinput absolute axis range.
  - Libei (EIS) Required fields:
    - `eis_socket_path`: The EIS socket path (e.g., "/run/user/1000/gamescope-0-ei").
  - Optional fields:
    - `use_win32_vk_code`: Whether to use Win32 Virtual-Key codes (VK_*). If `true`, keys use Win32 VK codes. The default value is `false`, which uses Linux evdev scan codes.

Create Linux controller for controlling applications on Linux. See [Control Methods](2.4-ControlMethods.md#linux).
...
- `contact`: Contact number
  ...
  - Android Native Controller: finger id (0 for first finger, valid range 0–15)
  - Win32 Controller: mouse button (0: left, 1: right, 2: middle)
  - MacOS Controller: mouse button (0: left, 1: right, 2: middle)
...
- `contact`: Contact number
  ...
  - Android Native Controller: finger id (0–15)
  - Win32 Controller: mouse button (0: left, 1: right, 2: middle)
  - MacOS Controller: mouse button (0: left, 1: right, 2: middle)
...
- `linux`: `type`, `screencap_method`, `input_method`, `wlr_socket_path`, `pw_socket_fd`, `pw_node_id`, `uinput_screen_width`, `uinput_screen_height`, `uinput_path`, `eis_socket_path`, `use_win32_vk_code`
...
- `node_name [out]`: Node name buffer
- `phase [out]`: Phase string buffer ("pre" / "post" / "repeat" / "context")
- `success [out]`: Whether succeeded
- `elapsed_ms [out]`: Total elapsed time in milliseconds
- `reco_id_list [out]`: Recognition ID array for each frame-to-frame comparison
- `reco_id_list_size [in, out]`: Size of the recognition ID array (input: buffer size, output: actual size)
- `roi [out]`: Actual ROI region used for comparison

Get wait freezes details. Obtained via `wf_id` from the callback. Each `reco_id` can be queried via `MaaTaskerGetRecognitionDetail` to get the drawn images.
...
Connect. If startup, protocol validation, or custom registration fails, the client rolls back registrations made by the current attempt and tries to shut down the remote session before returning `false`.
...
> - On Linux, the returned value represents the display number `n` in the Wayland socket name `wayland-<n>`. It is force-cast to `void*` at the API layer, so it is not a native window handle.
...
> - On Linux, the returned value is the full path of the Wayland socket (e.g. `/run/user/1000/wayland-0`).
...
> - On Linux, the returned value is the file name of the Wayland socket (e.g. `wayland-0`).

## MaaToolkitGamescope.h

> Linux (non-Android) only.

### MaaToolkitGamescopeInstanceListCreate

Create a gamescope instance list

### MaaToolkitGamescopeInstanceListDestroy

- `handle`: Instance list handle

Destroy a gamescope instance list

### MaaToolkitGamescopeInstanceFindAll

- `buffer [out]`: Output buffer

Enumerate gamescope instances into `buffer` (the list is empty when gamescope is not running).  
Each instance is keyed by a `gamescope-<n>` Wayland socket under `$XDG_RUNTIME_DIR` (falling back to `/run/user/<uid>` when unset); its PipeWire node ID is fetched via the `gamescope_pipewire` protocol, and its EIS socket path is derived as the same-named `gamescope-<n>-ei`.  
Sorted by display number in ascending order; stale sockets that cannot be connected to are dropped
...
Get instance at the specified index in the instance list

### MaaToolkitGamescopeInstanceGetDisplayNo

- `instance`: Instance

Get display number (the `n` in `gamescope-<n>`)

### MaaToolkitGamescopeInstanceGetPipeWireNodeId

- `instance`: Instance

Get PipeWire node ID (`0` means no capture node available); a non-zero value is usable as the `pw_node_id` config of the Linux controller (session-daemon node capture)

### MaaToolkitGamescopeInstanceGetEisSocketPath

- `instance`: Instance

Get EIS socket path (empty string means no EIS socket), usable as the `eis_socket_path` config of the Linux controller (Libei input)

## MaaToolkitPortal.h

> Linux (non-Android) only; calls on other platforms fail and return empty values (`nullptr` / `-1` / `0` / `false`).

### MaaToolkitPortalHelperCreate

Create a Portal helper object, used to complete the authorization flow through the `xdg-desktop-portal` ScreenCast interface and obtain PipeWire screencap stream parameters.

### MaaToolkitPortalHelperDestroy

- `helper`: Portal helper object

Destroy the Portal helper object and close the D-Bus session it holds

### MaaToolkitPortalHelperOpenStream

- `helper`: Portal helper object

Run a complete ScreenCast authorization flow (CreateSession → SelectSources → Start → OpenPipeWireRemote), selecting a single monitor. A system authorization dialog pops up during the process and waits for user confirmation; it fails if not confirmed within 30 seconds.

> Authorization options must be set before calling via `MaaToolkitPortalHelperSetPersist` / `MaaToolkitPortalHelperSetRestoreToken`.  
> On failure, the session created internally during this call is cleaned up.

### MaaToolkitPortalHelperGetPersist

- `helper`: Portal helper object

Get whether persistent authorization (persist) is currently requested, default `false`

### MaaToolkitPortalHelperSetPersist

- `helper`: Portal helper object
- `enable`: whether to request persistent authorization

Set whether to request persistent authorization; must be called before `MaaToolkitPortalHelperOpenStream`

### MaaToolkitPortalHelperGetPipeWireFD

- `helper`: Portal helper object

Get the PipeWire socket FD obtained from authorization (`-1` means not yet obtained), usable as the `pw_socket_fd` config of the Linux controller

### MaaToolkitPortalHelperGetPipeWireNodeID

- `helper`: Portal helper object

Get the PipeWire node ID obtained from authorization (`0` means not yet obtained), usable as the `pw_node_id` config of the Linux controller

### MaaToolkitPortalHelperGetRestoreToken

- `helper`: Portal helper object

Get the restore token (empty string means not obtained). When persistent authorization is requested, the portal issues it after successful authorization; persist it and pass it next time via `MaaToolkitPortalHelperSetRestoreToken` to avoid repeated authorization
...
Set the restore token; must be called before `MaaToolkitPortalHelperOpenStream`; passing a null pointer fails

...
Synchronously execute action logic for `entry`, returns node id. Returns `MaaInvalidId` on failure, or node id on success. You can get node details via `MaaTaskerGetNodeDetail`.

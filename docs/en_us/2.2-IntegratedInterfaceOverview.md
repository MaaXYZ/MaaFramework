...
- `config_json`: Linux controller config JSON.
    ...
    - PipeWire session-daemon node capture (gamescope etc.) Required fields:
        - `pw_node_id`: attach to a session-daemon node directly; discover the node with
          `MaaToolkitGamescopeNodeFindAll`.
...
### MaaToolkitGamescopeNodeListCreate

Create gamescope node list

### MaaToolkitGamescopeNodeListDestroy

- `handle`: Node list handle

Destroy gamescope node list

### MaaToolkitGamescopeNodeFindAll

- `buffer [out]`: Output buffer

Enumerate gamescope `Video/Source` nodes on the session PipeWire daemon into `buffer` (the list is empty when gamescope is not running)

### MaaToolkitGamescopeNodeListSize

- `list`: Node list

Get node list size

### MaaToolkitGamescopeNodeListAt

- `list`: Node list
- `index`: Index

Get node at the specified index in the node list

### MaaToolkitGamescopeNodeGetName

- `node`: Node

Get node name (always `gamescope`)

### MaaToolkitGamescopeNodeGetId

- `node`: Node

Get node ID, usable as the `pw_node_id` config of the Linux controller (session-daemon node capture)

### MaaToolkitGamescopeEisSocketListCreate

Create a gamescope EIS socket list

### MaaToolkitGamescopeEisSocketListDestroy

- `handle`: Socket list handle

Destroy a gamescope EIS socket list

### MaaToolkitGamescopeEisSocketFindAll

- `buffer [out]`: Output buffer

Enumerate EIS sockets named `gamescope-<n>-ei` under `$XDG_RUNTIME_DIR` (falling back to `/run/user/<uid>` when unset) into `buffer` (the list is empty when gamescope is not running).  
Sorted by display number; stale sockets left behind by a crashed gamescope may also be listed

### MaaToolkitGamescopeEisSocketListSize

- `list`: Socket list

Get socket list size

### MaaToolkitGamescopeEisSocketListAt

- `list`: Socket list
- `index`: Index

Get socket at the specified index in the socket list

### MaaToolkitGamescopeEisSocketGetPath

- `socket`: Socket

Get EIS socket path, usable as the `eis_socket_path` config of the Linux controller (Libei input)

...
### MaaContextGetCheckpoint

- `checkpoint_name`: Checkpoint name
- `buffer [out]`: Output buffer

Get the node name for checkpoint `checkpoint_name`, write to `buffer`. Returns `false` if checkpoint does not exist.

### MaaContextSetCheckpoint

- `checkpoint_name`: Checkpoint name
- `node_name`: Node name

Set checkpoint `checkpoint_name` to node `node_name`.

### MaaContextGetAllCheckpoints

- `buffer [out]`: Output buffer

Get all checkpoints as JSON object, write to `buffer`. The JSON format is `{"checkpoint_name": "node_name", ...}`.

### MaaContextMakeJumpNodes

- `jumpback_list`: Jumpback list
- `result [out]`: Output result list

Make jump nodes from `jumpback_list`. For each checkpoint name in `jumpback_list`, find the corresponding node name and add to result. Returns `true` on success.

...
- `dump_dir`: Directory where recording files were written by `MaaRecordControllerCreate` (contains `recording.jsonl` and screenshots)
...
- `dump_dir`: Directory where recording files are written (e.g. `recording.jsonl` and screenshots). Replay with `MaaReplayControllerCreate` using the recorded data.

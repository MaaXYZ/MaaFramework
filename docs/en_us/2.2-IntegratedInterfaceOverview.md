...
Note: for **newly created** nodes in `pipeline_override`, the default baseline is the framework built-in defaults, and it does not inherit from loaded `default_pipeline.json` (including `$Default` and type-specific defaults).
...
Get the **framework built-in default parameters** for the specified recognition type, serialize as JSON and write to `buffer`.  
This API does not read `default_pipeline.json` from loaded resources.
...
Get the **framework built-in default parameters** for the specified action type, serialize as JSON and write to `buffer`.  
This API does not read `default_pipeline.json` from loaded resources.

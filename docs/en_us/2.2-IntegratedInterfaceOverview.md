...
The returned JSON always contains a `type` field indicating the controller type. Depending on the type, it also includes the respective constructor parameters and state information.

Fields returned by each type:

- `adb`: `type`, `adb_path`, `adb_serial`, `screencap_methods`, `input_methods`, `agent_path`, `config`, `connected`, `image_resolution`
- `win32`: `type`, `hwnd`, `screencap_method`, `mouse_method`, `keyboard_method`, `connected`
- `playcover`: `type`, `address`, `uuid`, `connected`
- `gamepad`: `type`, `hwnd`, `gamepad_type`, `screencap_method`, `connected`
- `wlroots`: `type`, `wlr_socket_path`, `connected`
- `custom`: `type` (if the custom controller implements the `get_info` callback, additional fields from it will also be included)
- `dbg_carousel_image`: `type`, `path`, `image_count`, `image_index`, `resolution`, `connected`
- `dbg_replay_recording`: `type`, `record_count`, `record_index`, `connected`
...
> Note: On older Windows versions that don't support AF_UNIX (before Build 17063), `MaaAgentClientCreateV2` will automatically fall back to TCP mode.

...
### MaaControllerPostMouseMoveRelative

- `dx`: Horizontal move distance
- `dy`: Vertical move distance

Asynchronously move mouse relatively (Only supported by some controllers like Win32). This is an asynchronous operation that immediately returns an operation id. You can query the status via `MaaControllerStatus` and `MaaControllerWait`.

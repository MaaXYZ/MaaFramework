...
- Use `click_key`/`key_down`/`key_up` for digital buttons, and `touch_down`/`touch_move`/`touch_up` for sticks and triggers
- Feature flags include `MaaControllerFeature_UseMouseDownAndUpInsteadOfClick` and `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`

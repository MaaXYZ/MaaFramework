...
- `hWnd`: Window handle
- `screencap_method`: Screenshot method to use, see [Control Methods](2.4-ControlMethods.md#win32-screencap)

Create Gamepad controller for simulating a virtual Xbox 360 gamepad to control games that support gamepad input.

Notice

- Requires [ViGEmBus](https://github.com/nefarius/ViGEmBus/releases) driver installation
- Windows only
- Does not support `click`, `swipe`, `start_app`, `stop_app`, `input_text`, `scroll` operations
- Supports background operation
- Feature flag is `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`
- See [Control Methods](2.4-ControlMethods.md#gamepad-windows) for detailed usage

...
Get the current definition of task `node_name`. The returned JSON object contains an `attach` field that stores the additional JSON configuration of the node.
...
- `mouse_methods`: mouse input methods used
- `keyboard_methods`: keyboard input methods used
- `notify`: Callback for controller-related operations
...
Get current definition of task `node_name`. The returned JSON object contains a `raw` field that stores the original JSON configuration of the node.

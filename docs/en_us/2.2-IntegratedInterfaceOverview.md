...
- `recognition_data`: Pipeline Node object JSON

Asynchronously execute recognition. This is an asynchronous operation that immediately returns a task id. You can query the status via `MaaTaskerStatus` and `MaaTaskerWait`, and get recognition details via `MaaTaskerGetRecognitionDetail`.
...
- `action_data`: Pipeline Node object JSON

Asynchronously execute action. This is an asynchronous operation that immediately returns a task id. You can query the status via `MaaTaskerStatus` and `MaaTaskerWait`, and get action details via `MaaTaskerGetActionDetail`.

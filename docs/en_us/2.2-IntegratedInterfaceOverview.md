...
Asynchronously execute a relative movement. Currently supported by Win32 controller. Unsupported controllers may fail or return an invalid operation id. This is an asynchronous operation that immediately returns an operation id. You can query the status via `MaaControllerStatus` and `MaaControllerWait`.

...
- `roi`: ROI region, if NULL uses entire screen
- `other_param`: Optional advanced parameters JSON object containing `time`, `threshold`, `method`, `rate_limit`, `timeout`

Wait for screen to stabilize. `time` and `other_param.time` are mutually exclusive: both cannot be non-zero, and both cannot be zero.

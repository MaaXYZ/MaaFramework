...
| ScreenDC | `32` | 快 | 高 | 否 | 否 | |
| AutoForeground | `1 << 48` | N/A | N/A | 否 | 否 | 按顺序尝试 `DXGI_DesktopDup_Window -> GDI -> ScreenDC` |
| AutoBackground | `1 << 49` | N/A | N/A | 否 | 是 | 按顺序尝试 `FramePool -> PrintWindow` |

> [!NOTE]
>
> FramePool 和 PrintWindow 内置了伪最小化支持：当目标窗口被最小化时，会将窗口设为透明并开启点击穿透，以不激活的方式恢复窗口，从而在不打扰用户的情况下继续截图。`AutoBackground` 由于按顺序尝试这两种方式，也具备相同行为。
> 其他截图方式（包括 `AutoForeground`）在窗口最小化后无法获取有效内容，请避免窗口最小化。

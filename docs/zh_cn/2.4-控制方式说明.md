...
- **显示器捕获**（默认）：传入通过 `xdg-desktop-portal` 取得的合成器 PipeWire FD（`pw_socket_fd`）和节点 ID（`pw_node_id`）。
- **会话 daemon 节点捕获**：不传 FD，设置 `pw_node_id` 直连会话 PipeWire daemon 上的节点（如 gamescope 的 Video/Source 节点），
  节点 ID 可通过 `MaaToolkitGamescopeNodeFindAll` 发现。

使用 `Libei` 输入方式时，需要提供 `eis_socket_path` 指向合成器提供的 EIS socket，例如 gamescope 的 `/run/user/<uid>/gamescope-<n>-ei`。
...
Gamescope 控制器用于在 Linux 上控制 [Gamescope](https://github.com/ValveSoftware/gamescope) 微合成器中运行的应用程序（如
SteamOS 游戏模式或嵌套 gamescope 会话）。使用方式为在 `MaaLinuxControllerCreate` 的 `config_json` 中配置：

- `screencap_method`：`PipeWire`（会话 daemon 节点捕获）
- `pw_node_id`：gamescope 的 PipeWire 节点 ID，可通过 `MaaToolkitGamescopeNodeFindAll` 发现（gamescope 每个实例发布一个
  `Video/Source` 节点）；捕获画面为 gamescope 输出（游戏画面分辨率，如 1280x720）
- `input_method`：`Libei`
- `eis_socket_path`：gamescope 提供的 EIS socket 路径，形如 `/run/user/<uid>/gamescope-<n>-ei`
- `use_win32_vk_code`：可选，按键键码解释方式，见上方键盘输入说明

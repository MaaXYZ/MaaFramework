...
KWin 控制器用于在 Linux 上通过 PipeWire 和 `/dev/uinput` 实现截图与输入。之所以仍称为 "KWin 控制器"，是因为最初在该控制器开发阶段，主要基于 KWin 合成器进行测试与验证，此名称沿用至今。
...
1. 依赖组件：
   - `pipewire` (1.0+)
   - `xdg-desktop-portal`
2. 权限要求：当前用户必须具有 `/dev/uinput` 设备的读写权限。
   - **建议配置 udev 规则**：创建 `/etc/udev/rules.d/99-uinput.rules`，添加内容 `KERNEL=="uinput", MODE="0660", GROUP="input"`，并将当前用户加入 `input` 组，重启生效。

> [!TIP]
> 
> 只要系统的桌面环境实现了 XDG Screencast Portal 且内核启用了 uinput 模块，即可使用 KWin 控制器（如 GNOME）。

### KWin Screencap

KWin 控制器使用 PipeWire 与 `xdg-desktop-portal` 协议进行屏幕捕获。
> [!NOTE]
>
> - **授权交互**：首次调用截图功能时，桌面环境会触发原生的 ScreenCast 授权弹窗，用户需要手动点击“允许”。

### KWin Input

KWin 控制器通过内核级的虚拟输入设备进行控制。

### 键盘输入说明

KWin 控制器下，使用 MaaControllerPostKey{Down,Up} 传入的按键**默认**为 evdev 扫描码，定义见 [linux/input-event-codes.h](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/input-event-codes.h)。此行为与 [WlRoots 控制器](#wlroots-linux) 一致。

如需使用 Win32 Virtual-Key 键码（VK_*），在创建控制器时将 `MaaKWinControllerCreate` 的 `use_win32_vk_code` 参数置为 `true` 即可。开启后，`MaaControllerPostClickKey` / `MaaControllerPostKey{Down,Up}` 所接受的键码会被视为 Win32 VK 键码并在内部转换为 evdev 码。此开关在创建时一次性决定，不可运行时修改，便于从 Win32 控制器的代码中迁移按键逻辑到 KWin。

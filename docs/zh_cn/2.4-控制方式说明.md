...
| ScreenDC | `32` | 快 | 高 | 否 | 否 | |
| FramePoolWithPseudoMinimize | `64` | 极快 | 中 | 否 | 是 | Windows 10 1903+ 可用，支持伪最小化 |
| PrintWindowWithPseudoMinimize | `128` | 中 | 中 | 否 | 是 | 支持伪最小化 |

> [!NOTE]
>
> Windows 在窗口最小化后会停止绘制该窗口内容，大部分截图方式无法获取有效内容。
> 如需在最小化状态下截图，请使用带 `PseudoMinimize` 后缀的截图方式。这些方式会将窗口设为透明并开启点击穿透，以不激活的方式恢复窗口，从而在不打扰用户的情况下继续截图。

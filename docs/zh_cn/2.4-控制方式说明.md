...
> - Win32 还提供了一个独立于 `keyboard_method` 的后台按键守护能力：通过 `MaaControllerPostSetBackgroundManagedKeys` 声明受管键域后，命中的 `ClickKey` / `LongPressKey` / `KeyDown` / `KeyUp` 会自动改走后台守护路径，并在控制器空闲时继续纠偏。
> - 该后台按键守护能力兼容性非常低，目前仅验证用于终末地后台 WASD；默认会在受管键按下时发送一次 `F13` nudge，并且 `MaaControllerPostInactive` / stop / 队列自然结束时会释放全部受管键。
> - 后台按键守护器使用全局热键 ID 范围 2000–2255（`kManagedHotkeyBase + keycode`），若其他应用使用了重叠的热键 ID 可能产生冲突。

...
- **会话 daemon 节点捕获**：不传 FD，设置 `pw_node_id` 直连会话 PipeWire daemon 上的节点（如 gamescope 的 Video/Source 节点）。Gamescope 节点 ID 可通过 `MaaToolkitGamescopeInstanceFindAll` 发现。
...
使用 `Libei` 输入方式时，需要提供 `eis_socket_path` 指向合成器提供的 EIS socket，例如 gamescope 的 `/run/user/<uid>/gamescope-<n>-ei`。文本输入需要系统 `libei ≥ 1.6.0`。
...
在 [ProjectInterface V2](3.3-ProjectInterfaceV2协议.md) 的 Linux 控制器配置中，将 `use_win32_vk_code` 设置为 `true`，按键将使用 Win32 Virtual-Key 键码（VK_*）。默认值为 `false`，此时使用 Linux evdev 扫描码。

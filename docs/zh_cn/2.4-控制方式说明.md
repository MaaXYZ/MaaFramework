...
> [!NOTE]
>
> - **授权交互**：首次调用截图功能时，KDE 桌面会触发原生的 ScreenCast 授权弹窗，用户需要手动点击“允许”。

### KWin Input

KWin 控制器通过内核级的虚拟输入设备进行控制。
> [!NOTE]
> - **键盘输入**：支持全键盘模拟。默认将 MAA 下发的键码映射为 `linux/input-event-codes.h` 中的 `KEY_*` 宏定义发送。当 Linux 键码与 ASCII 字符发生冲突时，会优先保证 ASCII 字符的准确映射。
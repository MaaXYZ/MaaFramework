...
> - Win32 还提供了**后台受管键守护**（Background Managed Keys）：通过 `MaaControllerSetOption(ctrl, MaaCtrlOption_BackgroundManagedKeys, keycodes, sizeof(int32_t) * count)` 声明需要接管的虚拟键码数组。必须在连接前设置。声明成功后，命中的按键操作自动走后台守护路径，并在控制器空闲时持续修正按键状态。该能力兼容性极低，目前仅验证用于终末地后台 WASD。

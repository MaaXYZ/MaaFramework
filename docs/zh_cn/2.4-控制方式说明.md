...
| 类型 | API 值 | 说明 |
| --- | --- | --- |
| Xbox360 | `1` | Microsoft Xbox 360 Controller (有线) |
| DualShock4 | `2` | Sony DualShock 4 Controller (有线) |
...
| 功能 | 支持 | 说明 |
| --- | --- | --- |
| 截图 (screencap) | ✓ | 通过 Win32 截图方式（需要传入 hWnd） |
| 点击 (click) | ✗ | 手柄无点击概念，请使用 touch |
| 滑动 (swipe) | ✗ | 手柄无滑动概念，请使用 touch |
| 触摸 (touch_down/move/up) | ✓ | 用于控制摇杆和扳机 |
| 按键 (click_key/key_down/key_up) | ✓ | 用于控制数字按键 |
| 启动应用 (start_app) | ✗ | 不支持 |
| 停止应用 (stop_app) | ✗ | 不支持 |
| 文本输入 (input_text) | ✗ | 不支持 |
| 滚动 (scroll) | ✗ | 不支持 |
...
> [!NOTE]
>
> - Gamepad 控制器的特性标志包括 `MaaControllerFeature_UseMouseDownAndUpInsteadOfClick` 和 `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`。

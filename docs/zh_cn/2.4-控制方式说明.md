...
如果希望使用 Win32 Virtual-Key 键码（VK_*），可以通过 `MaaControllerSetOption(ctrl, MaaCtrlOption_WlRootsUseWin32VkCode, &enabled, sizeof(bool))` 开启（`enabled` 为 `true` 开启，`false` 关闭）。开启后，`MaaControllerPostClickKey` / `MaaControllerPostKey{Down,Up}` 所接受的键码将被视为 Win32 VK 键码，并在内部转换为 evdev 码。此选项仅对 WlRoots 控制器有效，便于从 Win32 控制器的代码中迁移按键逻辑到 WlRoots。

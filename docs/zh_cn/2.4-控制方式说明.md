...
> - [ProjectInterface V2](3.3-ProjectInterfaceV2协议.md) 仅支持配置 Win32 控制器的 input/screencap 方式。Adb 控制器的 input/screencap 由 MaaFramework 自动检测和选择最优方式，无需手动配置。
...
| PrintWindow | `16` | 使用 `PrintWindow(PW_CLIENTONLY|PW_RENDERFULLCONTENT)` 抓取；支持非最小化后台窗口，部分程序/图层窗口可能不兼容。 |

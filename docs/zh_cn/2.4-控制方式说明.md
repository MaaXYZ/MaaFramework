...
Gamepad 控制器用于模拟虚拟 Xbox 360 手柄，适合控制支持手柄操作的游戏。
...
| --- | --- |
| hWnd | 窗口句柄 |
| screencap_method | 截图方式，参见 [Win32 Screencap](#win32-screencap) |

### 支持的功能

| 功能 | 支持 | 说明 |
| --- | --- | --- |
| 截图 (screencap) | ✓ | 使用 Win32 截图方式 |
| 点击 (click) | ✗ | 手柄不支持鼠标点击 |
| 滑动 (swipe) | ✗ | 手柄不支持滑动 |
| 触摸 (touch_down/move/up) | ✓ | 映射到摇杆/扳机，见下表 |
| 按键按下 (key_down) | ✓ | 模拟手柄按键 |
| 按键抬起 (key_up) | ✓ | 模拟手柄按键 |
| 停止应用 (stop_app) | ✗ | 手柄不支持 |
| 启动应用 (start_app) | ✗ | 手柄不支持 |
| 文本输入 (input_text) | ✗ | 手柄不支持 |
| 滚动 (scroll) | ✗ | 手柄不支持 |

### 触摸操作映射

触摸操作通过 `contact` 参数映射到不同的手柄控件：

| contact | 映射控件 | 参数说明 |
| --- | --- | --- |
| 0 | 左摇杆 (Left Stick) | `x`: X 轴 (-32768~32767)，`y`: Y 轴 (-32768~32767) |
| 1 | 右摇杆 (Right Stick) | `x`: X 轴 (-32768~32767)，`y`: Y 轴 (-32768~32767) |
| 2 | 左扳机 (Left Trigger) | `pressure`: 扳机力度 (0~255) |
| 3 | 右扳机 (Right Trigger) | `pressure`: 扳机力度 (0~255) |

### 按键码

按键操作使用 XUSB 按键码，常用按键如下：

| 按键 | 值 | 说明 |
| --- | --- | --- |
| XUSB_GAMEPAD_DPAD_UP | 0x0001 | 方向键上 |
| XUSB_GAMEPAD_DPAD_DOWN | 0x0002 | 方向键下 |
| XUSB_GAMEPAD_DPAD_LEFT | 0x0004 | 方向键左 |
| XUSB_GAMEPAD_DPAD_RIGHT | 0x0008 | 方向键右 |
| XUSB_GAMEPAD_START | 0x0010 | Start 键 |
| XUSB_GAMEPAD_BACK | 0x0020 | Back 键 |
| XUSB_GAMEPAD_LEFT_THUMB | 0x0040 | 左摇杆按下 |
| XUSB_GAMEPAD_RIGHT_THUMB | 0x0080 | 右摇杆按下 |
| XUSB_GAMEPAD_LEFT_SHOULDER | 0x0100 | 左肩键 (LB) |
| XUSB_GAMEPAD_RIGHT_SHOULDER | 0x0200 | 右肩键 (RB) |
| XUSB_GAMEPAD_A | 0x1000 | A 键 |
| XUSB_GAMEPAD_B | 0x2000 | B 键 |
| XUSB_GAMEPAD_X | 0x4000 | X 键 |
| XUSB_GAMEPAD_Y | 0x8000 | Y 键 |

### 前置要求

1. 安装 [ViGEmBus](https://github.com/nefarius/ViGEmBus/releases) 驱动
2. 确保 `ViGEmClient.dll` 与程序在同一目录或系统路径中

> [!NOTE]
>
> - Gamepad 控制器的特性标志为 `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`，表示框架会使用 key_down + key_up 代替 click_key 操作。
> - 手柄控制器支持后台运行，无需窗口前台即可发送输入。

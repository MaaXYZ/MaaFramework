...
💡 通用 UI 会自动连接您的 Agent 子进程，并在执行到 `MyReco`/`MyAct` 时调用对应的识别/动作。
...
> [!NOTE]
> MaaFramework 提供多语言 API，但不建议采用全代码工作流，因为这会失去可视化节点编辑器、可视化调试器、通用 UI 等生态工具。大部分复杂需求都可以通过方案二的自定义扩展实现。
...
_简而言之，按文档编写一个 `interface.json`，告诉通用 UI 资源位置和可执行任务，以便它能正确运行项目。_

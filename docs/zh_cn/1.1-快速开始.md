...
在开始之前，我们先简单介绍一下 MaaFramework 的整体集成思路。MaaFramework 支持通过 Json 低代码编程，同时也提供了 C++, Python, Rust, C#, HTTP 接口以供开发者更灵活的操作。我们推荐的集成方式有以下几种：

- 完全依赖 Json 低代码编程

    简单快捷，推荐初次接触 MaaFramework 及编程小白使用。缺点是对于复杂逻辑的处理不够灵活。

    我们为此方式提供了[⭐项目模板](https://github.com/MaaXYZ/MaaPracticeBoilerplate)，可更方便的进行开发！

    该方式完全由 MaaFramework 通过 Pipeline 进行流程控制，无法实现较为复杂的转移逻辑或是状态记录。
...
- 自行编写代码控制流程，将自己的代码与 Json 低代码无缝结合
    ...
    该方式由自己的代码进行流程控制，调用 MaaFramework 进行截图、点击等操作，也可以独立执行 Pipeline 中任务的识别部分或是操作部分。
...
- 由 Json 掌控流程，但注册自定义任务处理复杂逻辑

    该方式完全由 MaaFramework 通过 Pipeline 进行流程控制，通过自定义识别或自定义任务来注入复杂逻辑。

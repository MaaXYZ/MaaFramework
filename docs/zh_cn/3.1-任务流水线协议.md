...
识别阶段是任务执行的起点，根据设定的算法和条件进行图像或特定模式的识别，判断是否需要执行接下来的动作。

- **识别算法 (`recognition`)**  
  选择识别算法类型，例如 `DirectHit`、`TemplateMatch`、`OCR` 等，详见 [算法类型](#算法类型) ,默认为 `DirectHit`。
  
- **识别结果反转 (`inverse`)**  
  如果 `inverse` 为 true，识别到的结果视为未识别，未识别到视为已识别。常用于特定的反向逻辑需求,默认为 false。

- **速率限制 (`rate_limit`)**  
  识别过程的速率限制，单位为毫秒，默认 1000。确保每轮识别至少消耗 `rate_limit` 毫秒，不足则 sleep 等待。

- **识别超时 (`timeout`)**  
  定义了识别阶段的超时时间，单位为毫秒。默认 20,000 毫秒。如果超时未识别到，将可能触发 `on_error`。

- **启用状态 (`enabled`)**  
  决定任务是否启用。如果 `enabled` 为 false，则跳过此任务，不进行识别和执行。

- **通知 (`focus`)**  
  如果 `focus` 设置为 true，则会产生回调消息，以便于监控任务的状态和进度。

### **2. 执行阶段**

在识别阶段匹配成功后，进入执行阶段。在这个阶段，会执行指定的动作，处理前后等待逻辑，以及动作执行结果。

- **动作 (`action`)**  
  定义要执行的操作类型，例如 `Click`、`Swipe`、`Text`、`Key` 等，默认为 `DoNothing`。这是任务的核心动作。

- **前置延迟 (`pre_delay`)**  
  识别成功后，执行动作前的延迟时间，单位为毫秒。默认 200 毫秒。

- **执行前等待画面静止 (`pre_wait_freezes`)**  
  在执行动作之前，系统会等待画面在指定时间内不发生较大变化。

- **后置延迟 (`post_delay`)**  
  执行动作后的等待时间，单位为毫秒，默认 200 毫秒。

- **执行后等待画面静止 (`post_wait_freezes`)**  
  在动作执行完成后，系统会等待画面静止。 

### **3. 后续任务**

任务执行后，根据设定的逻辑决定是否执行后续任务。如果识别或执行失败，还可以进行中断处理和错误处理。

- **后续任务 (`next`)**  
  定义后续任务列表，系统会按顺序识别并执行 `next` 列表中的第一个匹配到的任务。

- **中断任务 (`interrupt`)**  
  如果 `next` 列表中的任务没有匹配到，系统会按顺序尝试 `interrupt` 列表中的任务，并执行第一个匹配到的任务。在执行完 `interrupt` 后，系统会重新尝试识别 `next`。

- **错误处理 (`on_error`)**  
  如果超时或任务执行失败，系统会执行 `on_error` 列表中的任务。这是异常处理机制，确保任务在出错时不会中断。

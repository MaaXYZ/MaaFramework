...
> **兼容性说明：** 以上键名同时兼容不带 `$` 前缀的写法（如 `Default`、`TemplateMatch`、`Click`），但同一键名的两种写法不可同时存在，否则会报错。推荐使用 `$` 前缀写法。
...
1. 节点中直接定义的参数
2. Pipeline JSON 文件中 `$Default` 对象的默认参数（仅当前文件生效）
3. `default_pipeline.json` 中对应算法/动作类型的默认参数（仅当前 Bundle 生效）
4. `default_pipeline.json` 中 `$Default` 对象的默认参数（仅当前 Bundle 生效）
5. 框架内置的默认值

> 以上继承链适用于**从 Pipeline 文件加载的节点**。通过 `MaaResourceOverridePipeline` / `MaaContextOverridePipeline` 动态新增节点时，默认参数基线为框架内置默认值，不继承 `default_pipeline.json` 或文件级 `$Default`。
...
- 通过 `MaaResourceOverridePipeline` / `MaaContextOverridePipeline` 动态新增节点时，如需特定默认值，请在覆盖 JSON 中显式填写（不会自动继承 `default_pipeline.json` 或文件级 `$Default`）

#### 多 Bundle 加载行为

依次加载多个 Bundle 时：

- 各 Bundle 的 `default_pipeline.json` **仅对当前 Bundle 内的节点生效**，不会影响其他 Bundle 的节点
- **已加载的节点不会被后续 Bundle 的 `default_pipeline.json` 影响**
- 每个节点在首次加载时使用其所属 Bundle 的默认值，之后不再变化
...
{ "$Default": { "rate_limit": 100 } }   // 仅对 Bundle_Debug 内的节点生效
...
在 Pipeline JSON 文件中，可以定义一个 `$Default` 节点，作为**仅对当前文件生效**的默认属性。它的作用类似于 `default_pipeline.json` 中的 `Default` 对象，但作用范围限定在单个 JSON 文件内。
...
```jsonc
{
    ...
    "NodeA": {
        "recognition": "OCR"
        // rate_limit 继承 $Default 中的 500
        // pre_delay 继承 $Default 中的 100
    },
    "NodeB": {
        "recognition": "TemplateMatch",
        "template": "B.png",
        "rate_limit": 1000  // 覆盖 $Default 中的值
    }
}
```
...
- `$Default` 以 `default_pipeline.json` 中的 `Default` 为基础进行覆盖，然后作为当前文件内新节点的默认值
- 仅对当前文件中的**新节点**生效。若节点已在其他文件中定义过，则不受 `$Default` 影响（使用已有数据）
- `$Default` 本身以 `$` 开头，不会被注册为一个 Pipeline 节点

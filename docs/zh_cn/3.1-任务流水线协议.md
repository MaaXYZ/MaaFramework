...
> 若 Win32 控制器已通过 `MaaControllerPostSetBackgroundManagedKeys` 声明受管键域，则命中的键会自动走后台守护路径；若该键当前已处于后台保持状态，则 `ClickKey` 会失败。
...
> [!NOTE]
>
> 若 Win32 控制器已通过 `MaaControllerPostSetBackgroundManagedKeys` 声明受管键域，则命中的键会自动走后台守护路径；若该键当前已处于后台保持状态，则 `LongPressKey` 会失败。
...
> [!NOTE]
>
> 若 Win32 控制器已通过 `MaaControllerPostSetBackgroundManagedKeys` 声明受管键域，则命中的键会自动走后台守护路径，并在控制器空闲时继续保持和纠偏。
...
> [!NOTE]
>
> 若 Win32 控制器已通过 `MaaControllerPostSetBackgroundManagedKeys` 声明受管键域，则命中的键会自动走后台守护路径。

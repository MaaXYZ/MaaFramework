...
- `timeout`: *uint*  
    next 列表循环识别的超时时间，毫秒。可选，默认 20 * 1000（20秒）。  
...
- `contact`: *uint*  
  ...
  - Adb 控制器：表示手指编号（0 为第一根手指，1 为第二根手指，以此类推，最大不能超过10）
...
- `swipes`: *list<object,>*  
  ...
  - `contact`: *uint*  
    触点编号，用于区分不同的触控点。可选，默认使用该滑动在数组中的索引作为触点编号（从0开始）。
    - Adb 控制器：表示手指编号（0 为第一根手指，1 为第二根手指，以此类推，最大不能超过10）
    - Win32 控制器：表示鼠标按键编号（0 为左键，1 为右键，2 为中键，3 为 XBUTTON1，4 为 XBUTTON2）  

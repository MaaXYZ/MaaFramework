...
- `anchor`: *string* | *list<string, >* | *object*  
    ...
    - **对象**：`"anchor": {"A": "TargetNode", "B": ""}` - 将锚点 A 设置为 TargetNode，锚点 B 设置为当前节点（空字符串表示当前节点）
...
- 高级用法：允许将锚点映射到特定的目标节点，提供了更灵活的流程控制能力，类似于给 pipeline 引入了变量机制。

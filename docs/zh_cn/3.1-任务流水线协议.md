...
- `action`: *string*  
    ...
    可选的值：[`DoNothing`](#donothing) | [`Click`](#click) | [`LongPress`](#longpress) | [`Swipe`](#swipe) | [`MultiSwipe`](#multiswipe) | [`ClickKey`](#clickkey) | [`LongPressKey`](#longpresskey) | [`InputText`](#inputtext) | [`StartApp`](#startapp) | [`StopApp`](#stopapp) | [`StopTask`](#stoptask) | [`Command`](#command) | [`Custom`](#custom-1)  
    详见 [动作类型](#动作类型)。
...
- `checkpoint` : *string* | *list<string, >*  
    标记当前节点为指定名称的检查点。可选，默认空。  
    检查点用于配合 `jumpback` 字段实现跳转。当某个节点被成功识别和执行后，如果它的 `checkpoint` 字段非空，则会将该节点与检查点名称关联起来，后续节点可以通过 `jumpback` 跳转回来。
    例如:  
    ...
    当任务入口分别为 A1 和 A2 时，流水线为  
        - `A1` -> `B` -> `C` -> `A1` -> ...
        - `A2` -> `B` -> `C` -> `A2` -> ...
    可实现对 B, C 节点的复用。  

    另外，单个节点可关联多个 `checkpoint` 名称（数组）；不同节点可使用同一 `checkpoint` 名称，后执行的会覆盖掉之前的；注意 checkpoint 是 task 级的，不同 task 间不共享。

- `jumpback` : *string* | *list<string, >*  
    成功执行当前节点后，优先跳转到指定检查点名称对应的节点。可选，默认空。  
    与 next 逻辑一致，按顺序识别 jumpback 中的每个节点，只执行第一个识别到的。  
    若某个 checkpoint 未记录，则会被忽略。注意 checkpoint 是 task 级的，不同 task 间不共享。  
    jumpback 非空时，next 和 interrupt 不生效。

- `interrupt` : *string* | *list<string, >*  
    **（已在 5.x 版本中废弃，但保留兼容性，推荐使用 `checkpoint` + `jumpback` 替代）**  
...
- `is_sub`: *bool*  
    **（已在 2.x 版本中废弃，但保留兼容性，推荐使用 `checkpoint` + `jumpback` 替代）**  
    是否是子节点。可选，默认 false 。  
...
- `timeout`: *uint*  
    ...
    具体逻辑为 `while(!timeout) { foreach(jumpback OR (next + interrupt)); sleep_until(rate_limit); }` 。  
    注意：jumpback 非空时优先使用 jumpback，否则使用 next + interrupt。
...
1. UI 接收到框架发送的节点通知消息
2. 根据 message 查找 focus 中对应的模板字符串
3. 使用 details 中的数据替换模板中的占位符（如 `{name}`、`{task_id}`）
4. 将处理后的文本在界面中展示给用户，示例中即展示 `NodeA 开始执行，任务 ID: 12345`

### 更多消息类型

完整的回调消息列表和详细说明请参考 [回调协议](2.3-回调协议.md)。

...
| v5.10 | 时间字段（delay / timeout / duration / rate_limit 等）支持 `[min, max]` 随机区间; `TouchDown` / `KeyDown` 新增 `auto_up` 字段，任务停止、结束或控制器销毁时自动抬起仍按下的触点/按键 |

### 时间字段随机区间 **💡 v5.10**

以下时间类字段均支持填写单个整数，或 `[min, max]` 二元数组表示上下限。运行时会在闭区间内均匀随机取一个整数值：

- 节点通用字段：`rate_limit`、`timeout`、`pre_delay`、`post_delay`、`repeat_delay`
- `pre_wait_freezes` / `post_wait_freezes` / `repeat_wait_freezes` 的数值形式，以及 object 中的 `time`、`rate_limit`、`timeout`
- `LongPress` / `LongPressKey` 的 `duration`
- `Shell` 的 `shell_timeout`
- `MultiSwipe` 的 `starting`
- `Swipe` / `MultiSwipe` 的 `duration`、`end_hold`：每个元素可以是 `uint` 或 `[min, max]`。由于历史原因，`[a, b]` 仍表示两段滑动时长，单段随机请写成 `[[a, b]]`

`get_node_data` / `get_node_object` 会返回原始配置（单个整数或区间），随机发生在实际 sleep / 动作执行时。  
上下限相等的区间（如 `[100, 100]`）与标量语义相同，导出时会归一为标量。
...
- `rate_limit`: *uint* | *array<uint, 2>*  
    ...
    若为 `[min, max]`，运行时在区间内均匀随机取值。详见 [时间字段随机区间](#时间字段随机区间)。
...
- `expected`: *int* | *string* | *list\<int | string\>*  
    期望的分类下标或类别名称（字符串）。列表内可混合整数和字符串。可选，默认匹配所有结果。
...
例如画面中 **固定位置** 可能出现 猫、狗、老鼠，我们训练了支持该三分类的模型。  
...
- `contact`: *uint* **💡 v5.0**  
  ...
  - Android Native 控制器：表示手指编号（0 为第一根手指，取值 0–15，对应 Android MotionEvent pointer id）
...
- `duration`: *uint* | *array<uint, 2>* | *list<uint | array<uint, 2>,>*  
    ...
    每个元素支持单个值或 `[min, max]` 随机区间。由于 `[a, b]` 表示两段滑动时长，单段随机请写成 `[[a, b]]`。详见 [时间字段随机区间](#时间字段随机区间)。

- `end_hold`: *uint* | *array<uint, 2>* | *list<uint | array<uint, 2>,>*  
    滑动到终点后，额外等待一定时间再抬起，单位 ms。可选，默认 0。  
    每个元素支持单个值或 `[min, max]` 随机区间。由于 `[a, b]` 表示两段等待时长，单段随机请写成 `[[a, b]]`。详见 [时间字段随机区间](#时间字段随机区间)。
...
- `auto_up`: *bool* **💡 v5.10**  
    任务停止、结束或控制器销毁时是否自动抬起仍按住的触点。可选，默认 false 。  
    设为 true 后，若用户中途停止任务，或任务正常结束 / 控制器销毁时仍未执行对应的 `TouchUp`，将自动抬起该触点，避免设备保持按下导致无法正常操作。  
    若已执行对应的 `TouchUp`，不会再自动抬一次。
...
- `auto_up`: *bool* **💡 v5.10**  
    任务停止、结束或控制器销毁时是否自动抬起仍按下的按键。可选，默认 false 。  
    设为 true 后，若用户中途停止任务，或任务正常结束 / 控制器销毁时仍未执行对应的 `KeyUp`，将自动抬起该按键，避免设备保持按下导致无法正常操作。  
    若已执行对应的 `KeyUp`，不会再自动抬一次。

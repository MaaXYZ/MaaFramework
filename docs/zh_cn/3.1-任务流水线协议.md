...
- `labels`: *list<string, >*  
    标注，即每个分类的名字。可选。  
    仅影响调试图片及日志等。标签按 Pipeline、ONNX metadata、模型同名 JSON 的顺序读取；均未提供时生成 `Unknown_<class_id>`。一般来说不需要填写，使用 ONNX metadata 即可。

- `model`: *string*

    模型文件路径。使用 `model/detect` 文件夹的相对路径。必选。  
    路径应包含 `.onnx` 扩展名。使用 `Detect` head 的官方 Ultralytics 模型(包括 YOLO 和 RT-DETR )可通过 metadata 自动识别；其他模型必须使用同名 JSON 补全预处理和输出协议。无法确定完整协议时返回 `ModelError`。详见[神经网络目标检测模型协议](3.4-神经网络目标检测模型协议.md)。
    Ultralytics 模型导出参考 [NNDetect 食谱](https://github.com/MaaXYZ/MaaNeuralNetworkCookbook/tree/main/NeuralNetworkDetect)。

- `expected`: *int* | *list<int, >*  
    期望的分类下标。可选。  
    非空时，每个期望类别最多保留一个最高分结果；为空时，保留所有通过阈值和 NMS 的结果。
...
- `multi_label`: *boolean*
    ...
    是否为每个带分类分数的 proposal/query 保留全部合格类别。可选，默认 `true`。
    为 `false` 时先为每个 proposal/query 选择最高分类别；已携带分类下标的输出不受影响。

- `input_size`: *array<int, 2>*
    动态模型输入尺寸，顺序为 `[width, height]`。可选。
    仅覆盖 ModelProto 中的动态 H/W；静态维度始终优先。未填写时依次按 metadata `imgsz`、模型 JSON `Resize.size`、当前 ROI 尺寸解析。
...
注意这些值需要与模型实际输出相符。NMS 在模型同名 JSON 中配置，详见[神经网络目标检测模型协议](3.4-神经网络目标检测模型协议.md)。

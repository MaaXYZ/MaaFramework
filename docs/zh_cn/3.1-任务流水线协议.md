...
| v5.9 | `roi` / `target` 的 string 形式新增 `[Anchor]锚点名` 引用支持; 修复 action 失败时有无 `[JumpBack]` 的后续行为不一致（错误处理路径不触发回跳） |
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    注意：处于错误处理状态时不会回跳；仅在后续命中恢复为非错误处理状态，且 `next` 为空时才可能触发 `JumpBack`。  
    💡 v5.1 版本起新增支持带属性节点、 NodeAttr 形式对象，或两者结合的异质数组。详情请参考 [节点属性](#节点属性)。
...
一个节点的生命周期如下（不含 `anchor` / `[JumpBack]` 的跨节点控制，且不考虑动作失败分支）
...
> [!NOTE]
>
> 上图仅展示“单节点动作成功路径 + `next` 识别重试”主循环；动作失败后进入 `on_error` 的流程见 [执行逻辑](#执行逻辑)。  
> `anchor` / `[JumpBack]` 等跨节点控制详见 [节点属性](#节点属性)。  
> `need_to_stop` 会在每次动作执行前后检查，运行中置位后当前节点可能在动作阶段提前结束。

...
我们执行某个任务时（ MaaTaskerPostTask 接口设置入口名），会对其 next 列表中的 Node **依次** 进行识别（根据每个 Node 的 recognition 相关设置）  
且一旦匹配上了，则会退出 next 列表识别，转而去执行匹配上的 Node。类似遍历比较，一旦找到了，就直接 break 转而去执行找到的那个 Node。
...
- `detach`: *bool*  
    分离子进程，即不等待子进程执行完成，直接继续之后后面的节点。可选，默认 false。

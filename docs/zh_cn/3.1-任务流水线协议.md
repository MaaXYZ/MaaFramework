...
- `is_sub`: *bool*  
    *v2.0.0 已废弃，但仍保留兼容性*  
    该任务在 next 列表中时，`come_back` 相当于 `Head`。
...
- `name`: *string*  
    next 任务名。必选。
- `come_back`: *string*  
    在该 next 任务被完整执行完、最后一个 next 为空后，会接续上该 next 列表继续执行。可选，默认 `None`。  
    上例中，若识别命中 A - B - E - F，在这之后，会回来继续识别执行 [C, D, E, F, H] 及后续任务。  
    可选的值：  

  - `None`: 不使用该功能
  - `Head`: 回到该 next 列表头，例中接续上 [C, D, E, F, H]
  - `Current`: 回到当前位置，例中接续上 [E, F, H]
  - `Following`: 回到之后的位置，例中接续上 [F, H]

...
从 v5.1 版本新引入了节点属性功能，支持以两种方式设置节点属性
...
- 对象形式
    ...
    ```jsonc
    {
        "A": {
            "next": [
                "B",
                {   // 使用 object，设置 C 节点在此 next 列表中的 jump_back 属性
                    "name": "C",
                    "jump_back": true,
                },
                "D"
...
- 前缀形式
    ...
    ```jsonc
    {
        "A": {
            "next": [
                "B",
                "[JumpBack]C",  // 直接使用 `[]` 的属性前缀设置 jump_back 属性
                "D"
            ]
        }
    }
...
- `jump_back` / `[JumpBack]`  
    若命中该节点，在后续节点全部执行完成后，重新跳转到该节点来再次尝试识别。  
    例如: A: { next: ["B", "C", "[JumpBack]D", "E"] }  
    当 B, C 未识别到而识别到 D 时，会去完整的执行 D 及 D.next。但当 D 的流水线完全执行完毕后。会再次回到节点 A，继续尝试识别 B, C, D, E 。  
    该字段多可用于异常处理，例如 D 是识别 "网络断开提示框"，在点击确认并等待网络连接成功后，继续之前的节点流程。  
    （类似老版本 `is_sub`，区别在于仅在当前 next 中该节点是 `is_sub` 的。）

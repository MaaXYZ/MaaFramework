...
- `recognition` : *string*
    ...
    可选的值：[`DirectHit`](#directhit) | [`Multiple`](#multiple) | [`TemplateMatch`](#templatematch) | [`FeatureMatch`](#featurematch) | [`ColorMatch`](#colormatch) | [`OCR`](#ocr) | [`NeuralNetworkClassify`](#neuralnetworkclassify) | [`NeuralNetworkDetect`](#neuralnetworkdetect) | [`Custom`](#custom)
    详见 [算法类型](#算法类型)。
...
同一帧截图中，`and` 列表里的每个子识别都命中，才认为该节点识别成功。

该算法属性需额外部分字段：

- `and`: *list<any,>*
    子识别列表。必选。
    列表元素可以是 v1 格式的识别对象（如包含 `recognition` 字段，参数平铺在同一对象中），也可以是 v2 格式的识别对象（`{ "type": "...", "param": { ... } }`），两种格式允许混用。

- `sub_name`: *string*
    子识别别名。可选。
    若设置了 `sub_name`，则后续子识别可通过 `roi: "sub_name"` 引用该子识别的识别框作为 ROI（语法与 `roi: "节点名"` 一致）。
    注意：
  - 若存在多个同名 `sub_name`，对后续子识别而言，将以其之前出现的**最后一个**同名 `sub_name` 为准。
  - 在 `Multiple.and` 内，`roi: "xxx"` 按以下优先级解析：
    1. 先尝试将 `"xxx"` 解析为 `and` 列表中**更靠前**的子识别 `sub_name`；若找到但该子识别没有 box，则 ROI 退化为全屏。
    2. 若未命中 `sub_name`，则按 `roi: "节点名"` 的规则将 `"xxx"` 解析为之前执行过的节点名；若无法解析，则 ROI 退化为全屏。

- `box_index`: *int*
    指定输出哪个子识别的识别框（box）作为当前节点的识别框。可选，默认 0。
    解析期要求 `0 <= box_index < and.size`。
    若 `box_index` 指向的子识别没有 box，则该节点仍算命中，但整体 box 为空；当动作使用 `target: true` 时，目标退化为全屏（等价于 `[0, 0, 0, 0]`）。

示例（使用 `sub_name` + `roi: "sub_name"`）：

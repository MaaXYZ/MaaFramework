...
- `next` : *string* | *object* | *list<string | object, >*  
    接下来要执行的任务列表。可选，默认空。  
    按顺序识别 next 中的每个任务，只执行第一个识别到的。  
    若为 object，可设置更多参数，详见 [The Next](#the-next)。  

- `is_sub`: *bool*  
    *v2.0.0 已废弃，但仍保留兼容性*  
    该任务在 next 列表中时，`then_goto` 相当于 `Head`。
...
- `hit_limit`: *uint*
    命中次数上限。可选，默认 UINT_MAX。  
    当命中次数大于等于 `hit_limit` 时，相当于 `enabled` 为 false。
...
字段值为 string, object 或 list<string | object>，举例：
...
- `name`: *string*  
    next 任务名。必选。
- `then_goto`: *string*  
    在该 next 任务被完整执行完、最后一个 next 为空后，会跳转到该 next 列表继续执行。可选，默认 `None`。  
    上例中，若识别命中 A - B - E - F，在这之后，会回来继续识别执行 [C, D, E, F, H] 及后续任务。  
    可选的值：  

  - `None`: 不使用该功能
  - `Head`: 跳转到该 next 列表头，例中继续识别 [C, D, E, F, H]
  - `Current`: 跳转到当前位置，例中继续识别 [E, F, H]
  - `Following`: 跳转到之后的位置，例中继续识别 [F, H]

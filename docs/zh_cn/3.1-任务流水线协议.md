...
- `auto_up`: *bool* **💡 v5.10**  
    任务停止、结束或控制器销毁时是否自动抬起仍按住的触点。可选，默认 true 。  
    若用户中途停止任务，或任务正常结束 / 控制器销毁时仍未执行对应的 `TouchUp`，将自动抬起该触点，避免设备保持按下导致无法正常操作。  
    若已执行对应的 `TouchUp`，不会再自动抬一次。设为 false 可在任务结束后继续保持按下状态。
...
- `auto_up`: *bool* **💡 v5.10**  
    任务停止、结束或控制器销毁时是否自动抬起仍按下的按键。可选，默认 true 。  
    若用户中途停止任务，或任务正常结束 / 控制器销毁时仍未执行对应的 `KeyUp`，将自动抬起该按键，避免设备保持按下导致无法正常操作。  
    若已执行对应的 `KeyUp`，不会再自动抬一次。设为 false 可在任务结束后继续保持按下状态。

...
- `recognition` : *string*  
    ...
    可选的值：[`DirectHit`](#directhit) | [`TemplateMatch`](#templatematch) | [`FeatureMatch`](#featurematch) | [`ColorMatch`](#colormatch) | [`OCR`](#ocr) | [`NeuralNetworkClassify`](#neuralnetworkclassify) | [`NeuralNetworkDetect`](#neuralnetworkdetect) | [`Multiple`](#multiple) | [`Custom`](#custom)  
    详见 [算法类型](#算法类型)。
...
- `box_index`: *int*  
    ...
    需要满足 `0 <= box_index < and.size`。

- `sub_name`: *string*  
    子识别别名。可选。  
    后续子识别可通过 `roi: "sub_name"` 引用之前子识别的识别框作为 ROI；同名以最后一个为准。

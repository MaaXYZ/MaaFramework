...
- `recognition` : *string*  
    ...
    可选的值：`DirectHit` | `TemplateMatch` | `ColorMatch` | `OCR` | `Classify` | `Detect` | `Custom`  
    详见 [算法类型](#算法类型)。
...
- `hessian`: *int*  
    SURF 算法的 hessian 阈值，越大特征点的数量将越多。可选，默认 100.

- `ratio`: *double*  
    KNN 算法的距离比值，[0 - 1.0], 越大对点的匹配程序越宽松（更容易连线）。可选，默认 0.6.

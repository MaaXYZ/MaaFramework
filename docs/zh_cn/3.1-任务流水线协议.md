...
- `labels`: *list<string, >*  
    标注，即每个分类的名字。可选。  
    仅影响调试图片及日志等。标签按 Pipeline、模型同名 JSON、ONNX metadata 的顺序读取；均未提供时生成 `Unknown_<class_id>`。一般来说不需要填写，在模型同名 JSON 中配置或者使用 ONNX metadata 即可。

- `model`: *string*  
    模型文件路径。使用 `model/detect` 文件夹的相对路径。必选。  
    路径应包含 `.onnx` 扩展名。通过输入协议和输出张量协议组合支持 Maa YOLO Dense 兼容格式、YOLOv8、YOLO11、YOLO26、RT-DETR、RF-DETR、D-FINE 和 LW-DETR；只有历史 Maa YOLO Dense 兼容格式可省略同名模型描述文件。详见[神经网络目标检测模型协议](3.4-神经网络目标检测模型协议.md)。  
    YOLOv8、YOLO11训练参考 [NNDetect 食谱](https://github.com/MaaXYZ/MaaNeuralNetworkCookbook/tree/main/NeuralNetworkDetect)。
...
- `nms`: *string*  
    非极大值抑制策略。可选。  
    可选的值：`None` | `ClassAwareIoU` | `CandidateCoverage`。  
    配置优先级从高到低依次为 Pipeline、模型同名 JSON、输出协议默认值。一般来说不需要填写，在模型同名 JSON 中配置即可。

- `nms_threshold`: *double*  
    NMS 阈值。可选，取值范围为 [0, 1]。  
    仅填写该字段时，会覆盖当前有效 NMS 策略的阈值；策略为 `None` 时忽略。一般来说不需要填写，理由同上。

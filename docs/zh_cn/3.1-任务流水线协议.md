...
依次加载多个 Bundle 时，各 Bundle 的 `default_pipeline.json` 会按顺序合并（dict merge），后加载的同名字段会覆盖先加载的。合并基于顶层键（`Default`、各算法名、各动作名）独立进行。

**示例：** `Bundle_Base` → `Bundle_Special`
...
{
    "Default": { "rate_limit": 1000 },  // 覆盖，timeout 保留
    "OCR": { "threshold": 0.5 }
}
...
**示例：** 将调试节点分离到独立 Bundle，使用更短的延迟和更低的阈值
...
`debug` Bundle 的默认值会覆盖 `base` Bundle 的同名字段。若希望 `base` 节点不受影响，应在节点中显式设置参数（优先级最高）。

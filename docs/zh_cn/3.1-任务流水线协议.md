...
- `catch` : *string* | *list<string, >*  
    `next` 中全部未识别到时的候补任务列表，会执行出入栈操作。可选，默认空。  
    若 `next` 中的任务全部未识别到，则会按序识别该列表中的每个任务，并执行第一个识别到的。在后续任务全部执行完成后，重新跳转到该任务来再次尝试识别。  
    例如: A: { next: [B, C], catch: [D, E] }  

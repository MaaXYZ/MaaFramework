...
3. **节点属性解析（`NodeAttr`）**
   ...
   - `next` / `on_error` 中的节点可携带属性（如 `[JumpBack]`、`[Anchor]`）
   - 对于 `[Anchor]` 引用，运行时会先解析锚点到目标节点；若锚点未设置，则该候选节点会被跳过
...
5. **后继路由与状态更新**
   ...
   - `run_action` 返回后按动作结果路由：
     - 若 action 成功：进入该命中节点的 `next` 列表
     - 若 action 失败：进入该命中节点的 `on_error` 列表

   - 动作返回后会更新该节点定义的 `anchor`（无论 action 成功或失败）

6. **`JumpBack` 回跳**

   - 若命中节点在父节点列表中标记了 `jump_back`，会记录父节点回跳点
   - 当该分支执行完成且当前 `next` 为空、并且不在错误处理路径时，会回到父节点继续识别其 `next`

7. **错误处理与中断**

   - 当本轮 `next` 全部未命中时，会按 `rate_limit` 周期重试，直到命中或超时
   - 若本轮 `next` 中不存在可执行候选（例如都被禁用、锚点未解析到节点等），会直接按失败进入错误处理，不会进入超时重试循环
   - 当 `need_to_stop` 被置为 true（例如外部 `post_stop` 或执行 `StopTask`）时，任务循环会中断

### 终止条件

当满足以下任意条件时，任务流程终止：

- 正常结束：当前流程无后继可执行（`next` 为空，且 `JumpBack` 回跳栈为空）
- 主动停止：`need_to_stop == true`（例如 `StopTask` / `post_stop`）
- 错误结束：进入错误处理后仍无可继续路径（如 `on_error` 为空、错误处理阶段再次无可执行候选/识别超时；这类“未恢复”的错误处理流程不会触发 `JumpBack` 回跳）
...
| v5.9 | `roi` / `target` 的 string 形式新增 `[Anchor]锚点名` 引用支持; 修复 action 失败时有无 `[JumpBack]` 的后续行为不一致（错误处理路径不应该触发回跳） |
...
- `timeout`: *int*  
    ...
    一旦命中任一节点，会立即中断当前 `next` 列表的后续识别并进入动作阶段。

- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    在当前节点执行失败时（如识别超时、动作执行失败等），接下来会执行该列表中的节点。可选，默认空。  
    若该列表为空，节点失败后会直接进入错误处理终止路径。  
    注意：处于错误处理状态时，不会执行 `JumpBack` 回跳。只有 `on_error` 分支后续命中节点并将流程恢复为非错误处理状态，且之后出现 `next` 为空时，才可能触发回跳。  
    💡 v5.1 版本起新增支持带属性节点、 NodeAttr 形式对象，或两者结合的异质数组。详情请参考 [节点属性](#节点属性)。

- `anchor`: *string* | *list<string, >* | *object* **💡 v5.1**  
    锚点名称。可选，默认空。  
    当节点识别命中后，在 `run_action` 返回时会将该锚点名设置为对应的节点（无论动作成功或失败）。多个节点可设置同一个锚点名，后执行的会覆盖先执行的。  
...
- `repeat`: *uint* **💡 v5.3**  
    ...
    重复过程中不会因单次动作失败提前中止，最终以最后一次动作结果作为该节点动作结果。
...
一个节点的生命周期如下（不含 `anchor` / `[JumpBack]` 的跨节点控制）
...
> [!NOTE]
>
> 上图主要展示“单节点动作 + `next` 识别重试”主循环，跨节点控制补充如下：  
> `anchor`：命中节点后，在 `run_action` 返回时更新（无论动作成功或失败）。  
> `[JumpBack]`：命中带该属性的节点时先记录父节点；后续仅在 `next` 为空且当前不处于错误处理路径时才会回跳。  
> `need_to_stop`：在每次动作执行前后都会检查，因此运行中置位后，当前节点可能在动作阶段提前结束。
...
执行后会将任务置为停止状态（`need_to_stop`），中断后续识别循环。
...
在实现上，回跳点会先入栈；在当前分支 `next` 为空且不在错误处理路径时才会真正弹栈回跳。
若当前流程已进入错误处理（`on_error` 路径），不会执行回跳；需先通过后续命中恢复到非错误处理状态，之后在 `next` 为空时才可能回跳。

**执行流程**：

1. 父节点按顺序识别 `next` 列表中的节点
2. 若识别命中带有 `jump_back` 属性的节点，执行该节点及其后续节点链
3. 该节点链执行完毕后，若当前分支不处于错误处理路径，返回到父节点
...
- D 的节点链执行完毕后，若当前分支不处于错误处理路径，返回到节点 A
...
若锚点不存在或已被清除，则该候选节点会被跳过，不参与本轮识别。
锚点写入发生在 `run_action` 返回之后，而不是动作执行前。

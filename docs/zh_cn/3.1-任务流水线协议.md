...
- `target`: *true* | *string* | *array<int, 2>* | *array<int, 4>*  
    滚动目标的位置。可选，默认 true 。值同上述 `Click`.`target` 。  
  - Adb 控制器：在该位置执行滑动来模拟滚动
  - Win32 控制器：移动鼠标到该位置后发送滚轮事件

- `target_offset`: *array<int, 4>*  
    在 `target` 的基础上额外移动再作为滚动目标，四个值分别相加。可选，默认 [0, 0, 0, 0] 。

- `scroll_delta`: *array<int, 2>*  
    滚动距离 `[dx, dy]`。可选，默认 [0, 0] 。  
  - `dx`: 水平滚动距离，正值向右滚动，负值向左滚动
  - `dy`: 垂直滚动距离，正值向下滚动，负值向上滚动
  - Adb 控制器：会使用滑动来模拟滚动效果
  - Win32 控制器：对应鼠标滚轮事件的 `WHEEL_DELTA` 值

- `duration`: *uint*  
    滚动持续时间，单位毫秒。可选，默认 200 。  
  - Adb 控制器：滑动模拟滚动时的滑动时长
  - Win32 控制器：滚动后的等待时间

> [!NOTE]
> Adb 控制器的 Minitouch/Maatouch/AdbShell 输入方式不支持滚动操作，请使用 EmulatorExtras 方式。

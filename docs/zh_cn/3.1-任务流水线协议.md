...
执行通过 `MaaResourceRegisterCustomRecognizer` 接口传入的识别器句柄  

该算法属性需额外部分字段：

- `custom_recognition`: *string*  
    识别名，同注册接口传入的识别名。同时会通过 `MaaCustomRecognizerCallback`.`custom_recognition_name` 传出。必选。

- `custom_recognition_param`: *any*  
    识别参数，任意类型，会通过 `MaaCustomRecognizerCallback`.`custom_recognition_param` 传出。可选，默认空 json，即 `{}`

- `roi`: *array<int, 4>* | *string*  
    同 `TemplateMatch`.`roi`，会通过 `MaaCustomRecognizerCallback`.`roi` 传出。可选，默认 [0, 0, 0, 0]。
...
- `package`: *string*  
    启动入口。必选。  
    需要填入 packa name 或 activity，例如 `com.hypergryph.arknights` 或 `com.hypergryph.arknights/com.u8.sdk.U8UnityContext`。

...
### 场景描述
在Android设备的系统设置界面中，需完成以下操作：

- 定位并进入「存储」功能模块；
- 在「存储」界面内向下滑动，定位并进入底部的「游戏」入口。
...
2. 识别 **打开游戏** -> 失败
3. 识别 **打开存储** -> 成功 -> 执行点击操作
4. 返回到 **示例** 的 `next`和`interrupt` 列表
5. 识别 **打开游戏** -> 失败
6. 识别 **打开存储** -> 失败
7. 执行 **滑动** 操作
8. 返回到 **示例** 的 `next`和`interrupt` 列表
9. 识别 **打开游戏** -> 成功 -> 执行点击操作
10. **打开游戏** 的 `next` 列表为空，任务完成
...
3. 识别 **打开存储** -> 失败
4. 执行 **滑动** 操作
5. 返回到 **示例** 的 `next`和`interrupt` 列表
6. 识别 **打开游戏** -> 成功 -> 执行点击操作
7. **打开游戏** 的 `next` 列表为空，任务完成

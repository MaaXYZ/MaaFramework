...
```mermaid
    graph LR
        ...
        A -->|3.识别| C[识别存储]
        A -->|未进行| D[识别无障碍]
        C -->|4.识别成功,执行点击动作| E[跳转 识别存储 的 next 列表]
        E -->|5.识别| F[识别游戏]
        E -->|未进行| G[识别应用]
        F -->|6.识别成功,执行滑动动作| I[跳转 识别游戏 的 next 列表]
        I -->|7.列表为空| H[任务完成]
```
...
```mermaid
    graph LR
        ...
        C -->|4.识别失败,返回识别下一节点| A
        A -->|5.识别| D[识别无障碍]
        D -->|6.识别失败,next列表全部未命中| E{检查是否识别是否超时}
        E -->|检查为超时| F[任务超时]
        E -->|8.检查未超时| A
        A -->|9.二次识别| B
        B -->|10.识别成功,执行点击动作| H[跳转 识别显示 的 next 列表]
        H -->|11.next列表未空| G[任务完成]
```

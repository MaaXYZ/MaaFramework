...
| v5.10 | `TouchDown` / `KeyDown` 新增 `auto_up` 字段，任务停止、结束或控制器销毁时自动抬起仍按下的触点/按键 |
...
- `timeout`: *int*  
    ...
    *有一个**特殊情况**是，如果该节点是 `Task` 的第一个节点即 `Entry` 节点，`timeout` 属性还会对自身的 `recognition` 识别等待时间生效。*  
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    *有一个**特殊情况**是，如果该节点是 `Task` 的第一个节点即 `Entry` 节点，在自身识别失败后会进入自身的 `on_error` 列表。*  
    💡 v5.1 版本起新增支持带属性节点、 NodeAttr 形式对象，或两者结合的异质数组。详情请参考 [节点属性](#节点属性)。

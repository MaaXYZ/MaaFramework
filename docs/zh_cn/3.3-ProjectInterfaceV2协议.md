...
| 2026-8-19 | v2.10.0 | `welcome` 支持公告对象数组 |
...
- welcome `string | object[]` **💡 v2.10.0**  
  欢迎消息，在用户首次使用时弹窗显示，亦可作为公告使用。字符串写法支持文件路径、URL或直接文本，内容支持Markdown格式，并支持国际化（以`$`开头）。  
  v2.10.0 起也可使用对象数组声明多条公告，Client 应按数组顺序展示。数组不能为空，每个对象包含以下字段：  
  - label `string`  
    公告标题。可选，支持国际化（以`$`开头）；未设置时不显示标题。  
  - content `string`  
    公告内容。必填，支持文件路径、URL或直接文本，内容支持Markdown格式，并支持国际化（以`$`开头）。  
  ...
  Client 应记录完整的有序公告列表；公告增删、重排以及任一 `label` 或 `content` 更新均视为内容更新，并应再次提示用户。旧版 Client 可能无法解析数组写法，需要兼容旧版 Client 时请继续使用字符串写法。

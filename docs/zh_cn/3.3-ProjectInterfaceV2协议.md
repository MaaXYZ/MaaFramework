...
- **Seize**
  ...
  抢占式控制。通过 `SendInput`/前台聚焦进行输入；会抢占焦点、移动系统光标、阻塞用户输入，不能后台控制。
...
- **LegacyEvent**
  ...
  旧版 `mouse_event`/`keybd_event` 接口；依赖前台与系统光标、阻塞用户输入，实测兼容性一般，不能后台控制。
...
| 输入 | Seize | 否（需前台） | - | - | 需管理员权限 | 抢占焦点、移动系统光标、阻塞用户输入 |
| 输入 | SendMessage | 是 | - | - | 目标高完整性时需同级权限 | 同步消息，要求目标处理消息 |
| 输入 | PostMessage | 是 | - | - | 目标高完整性时需同级权限 | 异步消息，可能丢/延迟 |
| 输入 | LegacyEvent | 否（需前台） | - | - | 需管理员权限 | 旧 API，兼容性一般、阻塞用户输入 |
| 输入 | PostThreadMessage | 是 | - | - | 目标高完整性时需同级权限 | 目标线程须消息循环 |
| 输入 | SendMessageWithCursorPos | 是 | - | - | 需管理员权限 | 同步消息+移动光标+阻塞输入，适用于检测鼠标位置的游戏 |
| 输入 | PostMessageWithCursorPos | 是 | - | - | 需管理员权限 | 异步消息+移动光标+阻塞输入，适用于检测鼠标位置的游戏 |

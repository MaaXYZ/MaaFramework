...
> 该文件可以通过 [schema](https://github.com/MaaXYZ/MaaFramework/blob/main/tools/interface_v2.schema.json) 文件获得提示和校验功能
>
...
- SendMessageWithCursorPos `32`
  ...
  结合 `SendMessage` 与实际光标移动；通过 `SetCursorPos` 移动系统光标到目标位置后发送同步消息。专为原神等检测实际鼠标位置的游戏设计，可后台控制，同样受 UIPI/权限限制。

- PostMessageWithCursorPos `64`

  结合 `PostMessage` 与实际光标移动；通过 `SetCursorPos` 移动系统光标到目标位置后发送异步消息。专为原神等检测实际鼠标位置的游戏设计，可后台控制，同样受 UIPI/权限限制。

- SendMessageWithCursorPosAndBlockInput `128`

  在 `SendMessageWithCursorPos` 基础上增加 `BlockInput`；操作期间阻塞用户输入，防止干扰。需要管理员权限。

- PostMessageWithCursorPosAndBlockInput `256`

  在 `PostMessageWithCursorPos` 基础上增加 `BlockInput`；操作期间阻塞用户输入，防止干扰。需要管理员权限。
...
| 输入 | SendMessageWithCursorPos | 是 | - | - | 目标高完整性时需同级权限 | 同步消息+移动光标，适用于检测鼠标位置的游戏 |
| 输入 | PostMessageWithCursorPos | 是 | - | - | 目标高完整性时需同级权限 | 异步消息+移动光标，适用于检测鼠标位置的游戏 |
| 输入 | SendMessageWithCursorPosAndBlockInput | 是 | - | - | 需管理员权限 | 同步消息+移动光标+阻塞输入，防止干扰 |
| 输入 | PostMessageWithCursorPosAndBlockInput | 是 | - | - | 需管理员权限 | 异步消息+移动光标+阻塞输入，防止干扰 |

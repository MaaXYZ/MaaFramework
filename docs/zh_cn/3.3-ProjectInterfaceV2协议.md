...
- import `string[]` **💡 v2.2.0**  
    ...
    支持导入这些文件中的 `task`、`option`、`preset` **💡 v2.3.0**、`group` **💡 v2.4.0**、`global_option` 与 `setting` **💡 v2.8.0** 字段。  
...
```jsonc
{
  "option": {
    "战斗快捷键": {
      "type": "hotkey",
      "label": "$战斗快捷键",
      "description": "配置战斗相关按键，输出为 ClickKey 可用的虚拟按键码数组",
      "hotkeys": [
        {
          "name": "FightCombo",
          "label": "$连携攻击",
      ...
      "pipeline_override": {
        "AutoFight": {
          "attach": {
            "combo_keymap": "{FightCombo}",
            "skill_keymap1": "{FightSkill1}"
          }
        }
      }
    }
...
> **注意：** `{FightCombo}` 等占位符在最终写入 Pipeline 时应被替换为整数数组（如 `[69]`），以便直接用于 `ClickKey.key` 等字段。虚拟按键码取值请参考 [`ClickKey`](3.1-任务流水线协议.md#clickkey) 中各控制器的按键码表。

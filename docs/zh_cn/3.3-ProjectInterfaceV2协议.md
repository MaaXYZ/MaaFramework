...
- MaaPiCli
- [MFW-CFA]([https://github.com/SweetSmellFox/MFAAvalonia](https://github.com/overflow65537/MFW-PyQt6))
...
- option `record<string, object>`
  ...
  - cases `object[]`
    ...
    **注意：** 当 `type` 为 `"switch"` 时，仅支持两个 cases，且需遵循以下规则：
    ...
    - 其他 `case.name` 会被识别为 **No 选项**
    - Client 会根据 case 的 name 来匹配用户的 Y/N 输入

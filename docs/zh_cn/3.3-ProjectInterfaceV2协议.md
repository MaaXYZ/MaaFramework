...
- controller `object[]`  
  ...
  - type `'Adb' | 'Win32' | 'PlayCover' | 'Gamepad' | 'WlRoots'`  
    控制器类型，取值为 `Adb`、`Win32`（仅 Windows）、`PlayCover`（仅 macOS）、`Gamepad`（仅 Windows）和 `WlRoots`（仅 Linux）。
...
- import `string[]` **💡 v2.2.0**  
    ...
    支持导入这些文件中的 `task`、`option` 和 `preset` **💡 v2.3.0** 字段。导入文件本身不支持声明顶层 `group`；但导入的任务仍可通过 `task.group` 引用主 `interface.json` 中定义的分组。  

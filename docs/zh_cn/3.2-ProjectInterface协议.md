...
* name `string`
  ...
  项目唯一标识符，用作项目ID。如果字符串以 `$` 开头，表示是国际化字符串，需要从翻译文件中读取实际显示值。
...
* task `object[]`
  ...
  * name `string`
    ...
    任务显示名称，Client 会展示给用户进行选择。支持国际化字符串（以`$`开头）。
...
* option `record<string, object>`
  ...
  * cases `object[]`
    ...
    * name `string`
      ...
      选项显示名称，Client 会展示给用户进行选择。支持国际化字符串。

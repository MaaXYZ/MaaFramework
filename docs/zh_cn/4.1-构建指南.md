...
主工程默认只构建和安装 MaaFramework，不再构建或安装 `MaaPiCli`。如需构建 CLI，请先完成上述安装，再将 `source/MaaPiCli` 作为独立 CMake 工程配置。将 `<triplet>` 替换为主工程使用的 MaaDeps triplet，例如 `maa-x64-windows`、`maa-x64-linux` 或 `maa-arm64-osx`：
...
如只需要链接 `MaaPiCli::Core` 静态库，可在配置时添加 `-DMaaPiCli_BUILD_EXECUTABLE=OFF`。

...
注意：对 `pipeline_override` 中**新创建**的节点，默认参数基线为框架内置默认值，不继承已加载资源中的 `default_pipeline.json`（包括 `$Default` 与类型默认参数）。
...
获取指定识别类型的**框架内置默认参数**，序列化为 JSON 写入到 `buffer`。  
该接口不读取已加载资源中的 `default_pipeline.json`。
...
获取指定动作类型的**框架内置默认参数**，序列化为 JSON 写入到 `buffer`。  
该接口不读取已加载资源中的 `default_pipeline.json`。

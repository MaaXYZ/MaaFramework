...
- `config_json`: Linux 控制器配置 JSON。
    ...
    - PipeWire 会话 daemon 节点捕获（gamescope 等）必填字段:
        - `pw_node_id`: 直连会话 daemon 节点，可用 `MaaToolkitGamescopeNodeFindAll` 发现节点。
    - UInput 可选字段:
...
枚举会话 PipeWire daemon 上的 gamescope `Video/Source` 节点，写入到 `buffer`（gamescope 未运行时列表为空）
...
获取节点名称（固定为 `gamescope`）

### MaaToolkitGamescopeNodeGetId

- `node`: 节点

获取节点 ID，可用于 Linux 控制器的 `pw_node_id` 配置（会话 daemon 节点直连）

### MaaToolkitGamescopeEisSocketListCreate

创建 gamescope EIS socket 列表

### MaaToolkitGamescopeEisSocketListDestroy

- `handle`: socket 列表句柄

销毁 gamescope EIS socket 列表

### MaaToolkitGamescopeEisSocketFindAll

- `buffer [out]`: 输出缓冲区

枚举 `$XDG_RUNTIME_DIR`（未设置时回退 `/run/user/<uid>`）下 `gamescope-<n>-ei` 命名的 EIS socket，写入到 `buffer`（gamescope 未运行时列表为空）。  
按 display 号排序；gamescope 崩溃遗留的 stale socket 也可能被列出
...
获取 socket 列表中指定索引的 socket

### MaaToolkitGamescopeEisSocketGetPath

- `socket`: socket

获取 EIS socket 路径，可用于 Linux 控制器的 `eis_socket_path` 配置（Libei 输入）

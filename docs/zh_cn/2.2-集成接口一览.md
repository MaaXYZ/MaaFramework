...
创建 Gamepad 控制器，用于模拟虚拟 Xbox 360 手柄来控制支持手柄操作的游戏。

注意

- 需要安装 [ViGEmBus](https://github.com/nefarius/ViGEmBus/releases) 驱动
- 仅支持 Windows 平台
- 不支持 `click`、`swipe`、`start_app`、`stop_app`、`input_text`、`scroll` 操作
- 支持后台运行
- 特性标志为 `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`
- 详细使用方法参见 [控制方式说明](2.4-控制方式说明.md#gamepad-windows)

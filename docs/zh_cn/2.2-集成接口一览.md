...
- `roi`: ROI 区域，为 NULL 时使用整个屏幕
- `wait_freezes_param`: 可选的高级参数 JSON 对象，包含以下字段：
  - `time`: 等待时间（毫秒）
  - `target`: 目标区域，支持 `Self`、`PreTask`、`Region` 等类型（与 `roi` 互斥）
  - `threshold`: 相似度阈值（默认 0.95）
  - `method`: 比对方法（默认为模板匹配默认方法）
  - `rate_limit`: 截图间隔（毫秒，默认 1000）
  - `timeout`: 超时时间（毫秒，默认 20000）

等待画面静止。`time` 和 `wait_freezes_param.time` 互斥：两者不能同时为非零，也不能同时为零。`roi` 和 `wait_freezes_param.target` 互斥：不能同时指定。

...
- `keycodes`: Win32 后台按键守护器接管的虚拟键码数组
- `count`: `keycodes` 数组长度，必须大于 0

异步声明 Win32 后台受管键域。这是一个异步操作，会立即返回一个操作 id，可通过 `MaaControllerStatus` 和 `MaaControllerWait` 查询状态。  
声明成功后，现有 `MaaControllerPostClickKey`、`MaaControllerPostKeyDown`、`MaaControllerPostKeyUp` 以及 pipeline 中的 `ClickKey`、`LongPressKey`、`KeyDown`、`KeyUp` 会对命中的键自动走后台守护路径。  
该能力仅支持 Win32 控制器；其他控制器调用会失败。  
该能力兼容性非常低，目前仅验证用于终末地后台 WASD，并在受管键按下时默认附带一次 `F13` nudge。

...
- BackgroundManagedKeys  
    声明 Win32 后台受管键域。必须在连接前设置，连接后设置会失败。声明成功后，现有 `MaaControllerPostClickKey`、`MaaControllerPostKeyDown`、`MaaControllerPostKeyUp` 以及 pipeline 中的 `ClickKey`、`LongPressKey`、`KeyDown`、`KeyUp` 会对命中的键自动走后台守护路径。该选项仅支持 Win32 控制器；其他控制器设置会失败。  

...
- 使用 `click_key`/`key_down`/`key_up` 控制数字按键，使用 `touch_down`/`touch_move`/`touch_up` 控制摇杆和扳机
- 特性标志包括 `MaaControllerFeature_UseMouseDownAndUpInsteadOfClick` 和 `MaaControllerFeature_UseKeyboardDownAndUpInsteadOfClick`

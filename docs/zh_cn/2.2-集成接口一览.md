...
异步执行识别。这是一个异步操作，会立即返回一个任务 id，可通过 `MaaTaskerStatus` 和 `MaaTaskerWait` 查询状态，通过 `MaaTaskerGetRecognitionDetail` 获取识别详情。
...
异步执行操作。这是一个异步操作，会立即返回一个任务 id，可通过 `MaaTaskerStatus` 和 `MaaTaskerWait` 查询状态，通过 `MaaTaskerGetActionDetail` 获取操作详情。

...
注册名为 `name` 的自定义识别器 `recognition`。名称不得为空。自定义识别器和自定义操作共享一个区分大小写的命名空间。若名称为空或已被任一类型注册，则返回 `false` 且保留已有注册；如需替换，应先移除原注册。
...
注册名为 `name` 的自定义操作 `action`。名称不得为空。自定义识别器和自定义操作共享一个区分大小写的命名空间。若名称为空或已被任一类型注册，则返回 `false` 且保留已有注册；如需替换，应先移除原注册。
...
> 支持多指触控：`contact` 为手指编号（0 为第一根手指，取值 0–15）。外部库的 `TouchArgs` 必须携带 `contact`。
...
- `read_path`: 图片目录（或单个图片文件）路径。连接时加载所有图片，截图时轮播。所有输入操作直接返回成功。

创建调试控制器

### MaaReplayControllerCreate

- `recording_path`: 录制 JSONL 文件路径，由 `MaaRecordControllerCreate` 写入。截图路径基于该文件所在目录解析。

创建回放控制器

### MaaRecordControllerCreate

- `inner`: 内层控制器，所有操作将转发到该控制器。录制控制器不取得 `inner` 的所有权。
- `recording_path`: 录制 JSONL 文件输出路径。截图会保存到同目录下的 `{stem}-Screenshot` 文件夹。可使用 `MaaReplayControllerCreate` 回放录制数据。

创建录制控制器，包装已有控制器并记录所有操作
...
- `config_json`: Linux 控制器配置 JSON。
  - 必填字段
    - `screencap_method`: 要使用的截图方式, 详见 [控制方式说明](2.4-控制方式说明.md#linux-screencap)。
    - `input_method`: 要使用的输入方式, 详见 [控制方式说明](2.4-控制方式说明.md#linux-input)。
  - Wlroots 必填字段:
    - `wlr_socket_path`: Wayland 套接字路径，如 `/run/user/1000/wayland-0`。
  - PipeWire 显示器捕获（经 xdg-desktop-portal）必填字段:
    - `pw_socket_fd`: 来自 ScreenCast portal 的 PipeWire socket FD。
    - `pw_node_id`: PipeWire 节点 ID。
  - PipeWire 会话 daemon 节点捕获（gamescope 等）必填字段:
    - `pw_node_id`: 直连会话 daemon 节点，可用 `MaaToolkitGamescopeInstanceFindAll` 发现节点。
  - UInput 可选字段:
    - `uinput_path`: uinput 设备节点路径，默认 `/dev/uinput`。
    - `uinput_screen_width`: uinput 绝对坐标轴范围的屏幕宽度（像素）。
    - `uinput_screen_height`: uinput 绝对坐标轴范围的屏幕高度（像素）。
  - Libei (EIS) 必填字段:
    - `eis_socket_path`: EIS socket 路径，如 `/run/user/1000/gamescope-0-ei`。
  - 可选字段:
    - `use_win32_vk_code`: 是否使用 Win32 Virtual-Key 键码（VK_*）。为 `true` 时，按键将使用 Win32 VK 键码。默认值为 `false`，此时使用 Linux evdev 扫描码

创建 Linux 控制器，用于在 Linux 上控制应用程序。详见 [控制方式说明](2.4-控制方式说明.md#linux)。
...
- `contact`: 触点编号
  ...
  - Android Native 控制器：手指编号（0 为第一根手指，取值 0–15）
  - Win32 控制器：鼠标按键（0：左键，1：右键，2：中键）
  - MacOS 控制器：鼠标按键（0：左键，1：右键，2：中键）
- `pressure`: 触点力度
...
- `node_name [out]`: 节点名缓冲区
- `phase [out]`: 阶段字符串缓冲区（"pre" / "post" / "repeat" / "context"）
- `success [out]`: 是否成功
- `elapsed_ms [out]`: 总耗时（毫秒）
- `reco_id_list [out]`: 识别号数组，每次画面对比的识别 ID
- `reco_id_list_size [in, out]`: 识别号数组大小（输入时表示缓冲区大小，输出时表示实际大小）
- `roi [out]`: 实际比较的 ROI 区域

获取等待画面静止的详细信息。通过回调中的 `wf_id` 获取，各 `reco_id` 可通过 `MaaTaskerGetRecognitionDetail` 查询对应的 draw 图片。
...
连接。若启动、协议校验或自定义注册失败，客户端会回滚本次连接已注册的自定义项，并在返回 `false` 前尝试关闭远端会话。
...
注册名为 `name` 的自定义识别器 `recognition`。名称不得为空。自定义识别器和自定义操作共享一个区分大小写的命名空间。若名称为空或已被任一类型注册，则返回 `false` 且保留已有注册。
...
注册名为 `name` 的自定义操作 `action`。名称不得为空。自定义识别器和自定义操作共享一个区分大小写的命名空间。若名称为空或已被任一类型注册，则返回 `false` 且保留已有注册。
...
> - Linux 下，返回值语义为 Wayland socket 名 `wayland-<n>` 中的 display 号 `n`，在接口层会强制转换为 `void*` 返回，因此它不是原生窗口句柄。
...
> - Linux 下，返回值语义为 Wayland socket 的完整路径（如 `/run/user/1000/wayland-0`）。
...
> - Linux 下，返回值语义为 Wayland socket 的文件名（如 `wayland-0`）。

## MaaToolkitGamescope.h

> 仅 Linux（非 Android）可用。
...
枚举 gamescope 实例，写入到 `buffer`（gamescope 未运行时列表为空）。  
实例以 `$XDG_RUNTIME_DIR`（未设置时回退 `/run/user/<uid>`）下 `gamescope-<n>` 命名的 Wayland socket 为键；每个实例经 `gamescope_pipewire` 协议获取 PipeWire 节点 ID，并派生同名 `gamescope-<n>-ei` EIS socket 路径。  
按 display 号升序排序；无法连接的 stale socket 会被丢弃
...
获取 display 号（`gamescope-<n>` 的 `n`）

### MaaToolkitGamescopeInstanceGetPipeWireNodeId

- `instance`: 实例

获取 PipeWire 节点 ID（`0` 表示无可用截图节点），非零时可用于 Linux 控制器的 `pw_node_id` 配置（会话 daemon 节点直连）

### MaaToolkitGamescopeInstanceGetEisSocketPath

- `instance`: 实例

获取 EIS socket 路径（空串表示无 EIS socket），可用于 Linux 控制器的 `eis_socket_path` 配置（Libei 输入）

## MaaToolkitPortal.h

> 仅 Linux（非 Android）可用，其他平台调用会失败并返回空值（`nullptr` / `-1` / `0` / `false`）。

### MaaToolkitPortalHelperCreate

创建 Portal 辅助对象，用于通过 `xdg-desktop-portal` 的 ScreenCast 接口走完授权流程，取得 PipeWire 截图流参数。

### MaaToolkitPortalHelperDestroy

- `helper`: Portal 辅助对象

销毁 Portal 辅助对象，并关闭其持有的 D-Bus 会话

### MaaToolkitPortalHelperOpenStream

- `helper`: Portal 辅助对象

执行一次完整的 ScreenCast 授权流程（CreateSession → SelectSources → Start → OpenPipeWireRemote），选择单个显示器。过程中会弹出系统授权对话框等待用户确认，超过 30 秒未确认则失败。

> 需在调用前通过 `MaaToolkitPortalHelperSetPersist` / `MaaToolkitPortalHelperSetRestoreToken` 设置授权选项。  
> 失败时会清理本次已在内部创建的会话。
...
设置是否请求持久授权，需在 `MaaToolkitPortalHelperOpenStream` 前调用

### MaaToolkitPortalHelperGetPipeWireFD

- `helper`: Portal 辅助对象

获取授权得到的 PipeWire socket FD（`-1` 表示尚未取得），可用于 Linux 控制器的 `pw_socket_fd` 配置

### MaaToolkitPortalHelperGetPipeWireNodeID

- `helper`: Portal 辅助对象

获取授权得到的 PipeWire 节点 ID（`0` 表示尚未取得），可用于 Linux 控制器的 `pw_node_id` 配置

### MaaToolkitPortalHelperGetRestoreToken

- `helper`: Portal 辅助对象

获取 restore token（空串表示未取得）。请求持久授权时，授权成功后会由 portal 下发，持久化保存后可在下次通过 `MaaToolkitPortalHelperSetRestoreToken` 传入以免去重复授权
...
设置 restore token，需在 `MaaToolkitPortalHelperOpenStream` 前调用；传入空指针会失败

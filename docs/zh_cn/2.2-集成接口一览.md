...
- `hWnd`: 窗口句柄
- `screencap_method`: 使用的截图方式
- `mouse_method`: 使用的鼠标输入方式
- `keyboard_method`: 使用的键盘输入方式
...
获取检查点 `checkpoint_name` 对应的节点名，写入到 `buffer`。如果检查点不存在则返回 `false`。
...
设置检查点 `checkpoint_name` 对应的节点为 `node_name`。

### MaaContextGetAllCheckpoints

- `buffer [out]`: 输出缓冲区

获取所有检查点，以 JSON 对象格式写入 `buffer`。JSON 格式为 `{"checkpoint_name": "node_name", ...}`。

### MaaContextMakeJumpNodes

- `jumpback_list`: jumpback 列表
- `result [out]`: 输出结果列表

根据 `jumpback_list` 生成跳转节点列表。对于 `jumpback_list` 中的每个检查点名，查找对应的节点名并添加到结果中。成功返回 `true`。

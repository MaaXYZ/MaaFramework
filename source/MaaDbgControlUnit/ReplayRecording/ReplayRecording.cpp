...
bool ReplayRecording::click(int x, int y)
{
    ...
    //     LogError << "record click is not match" << VAR(param.x) << VAR(param.y) << VAR(x) << VAR(y)
    //              << VAR(record.raw_data);
    //     return false;
    // }
...
bool ReplayRecording::swipe(int x1, int y1, int x2, int y2, int duration)
{
    ...
    // if (param.x1 != x1 || param.y1 != y1 || param.x2 != x2 || param.y2 != y2 || param.duration != duration) {
    //     LogError << "record swipe is not match" << VAR(param.x1) << VAR(param.y1) << VAR(param.x2) << VAR(param.y2)
    //              << VAR(param.duration) << VAR(x1) << VAR(y1) << VAR(x2) << VAR(y2) << VAR(duration)
    //              << VAR(record.raw_data);
    //     return false;
    // }
...
bool ReplayRecording::touch_down(int contact, int x, int y, int pressure)
{
    ...
    if (param.contact != contact || param.x != x || param.y != y || param.pressure != pressure) {
        LogError << "record touch_down is not match" << VAR(param.contact) << VAR(param.x) << VAR(param.y)
                 << VAR(param.pressure) << VAR(contact) << VAR(x) << VAR(y) << VAR(pressure) << VAR(record.raw_data);
        return false;
    }
...
bool ReplayRecording::touch_move(int contact, int x, int y, int pressure)
{
    ...
    if (param.contact != contact || param.x != x || param.y != y || param.pressure != pressure) {
        LogError << "record touch_move is not match" << VAR(param.contact) << VAR(param.x) << VAR(param.y)
                 << VAR(param.pressure) << VAR(contact) << VAR(x) << VAR(y) << VAR(pressure) << VAR(record.raw_data);
        return false;
    }

...
#define ReturnHwnd(func_tag)                                                   \
  LHGArgToJsonBegin(func_tag, ret, true)                                       \
    value = std::format("{:#018x}",                                            \
                        reinterpret_cast<size_t>(std::get<index>(arg)));       \
    LHGArgToJsonMiddle()                                                       \
    b.type("string");                                                          \
  LHGArgToJsonEnd()
...
LHGArgToJsonBegin(maa::func_type_MaaGetImageEncoded, ret, true)
  auto size = MaaGetImageEncodedSize(
      std::get<maa::func_type_MaaGetImageEncoded::_0_handle::index>(arg));
...
LHGArgFromJsonBegin(maa::func_type_MaaSetImageEncoded, _1_data, true)
  ...
  std::get<maa::func_type_MaaSetImageEncoded::_2_size::index>(arg) =
      data.size();
  LHGArgFromJsonMiddle()
  b.type("string");
LHGArgFromJsonEnd()
...
#define ApiCallback(func_tag, cb_tag, ctx_tag)                                 \
  ...
  LHGArgFromJsonBegin(func_tag, cb_tag, true)                                  \
    ...
    auto manager =                                                             \
        provider.get<CallbackManager<maa::callback_MaaAPICallback>, void>();   \
...
ApiCallback(maa::func_type_MaaWin32ControllerCreate, _2_callback,
            _3_callback_arg);
ApiCallback(maa::func_type_MaaAdbControllerCreateV2, _5_callback,
            _6_callback_arg);
ApiCallback(maa::func_type_MaaThriftControllerCreate, _4_callback,
            _5_callback_arg);
ApiCallback(maa::func_type_MaaDbgControllerCreate, _4_callback,
            _5_callback_arg);
...
LHGArgFromJsonBegin(maa::func_type_MaaRegisterCustomRecognizer, _2_recognizer,
                    true)
  std::string id = value.as_string();
  auto manager =
      provider
          .get<CallbackManager<maa::callback_CustomRecognizerAnalyze>, void>();
  auto ctx = manager->query(id);
  if (!ctx.get()) {
    return false;
  }
  ...
  std::get<
      maa::func_type_MaaRegisterCustomRecognizer::_3_recognizer_arg::index>(
      arg) = ctx.get();
  LHGArgFromJsonMiddle()
  b.type("string");
LHGArgFromJsonEnd()
...
LHGArgFromJsonBegin(maa::func_type_MaaRegisterCustomAction, _2_action, true)
  ...
  auto run_manager =
      provider.get<CallbackManager<maa::callback_CustomActionRun>, void>();
  auto run_ctx = run_manager->query(run_id);
  if (!run_ctx.get()) {
    return false;
  }
  auto stop_manager =
      provider.get<CallbackManager<maa::callback_CustomActionStop>, void>();
  auto stop_ctx = stop_manager->query(stop_id);
  if (!stop_ctx.get()) {
    return false;
  }
  auto ctx =
      provider.get<DataManager, void>()->alloc<pri_maa::custom_action_context>(
  ...
  std::get<index>(arg) = &api;
  std::get<maa::func_type_MaaRegisterCustomAction::_3_action_arg::index>(arg) =
      ctx;
  LHGArgFromJsonMiddle()

                                                        ...
                                                        const json::value& adb_config) const
{
    ...
    // TODO: fix as is
    std::shared_ptr<MAA_ADB_CTRL_UNIT_NS::DeviceListAPI> mgr;
    MAA_ADB_CTRL_UNIT_NS::create_device_list_obj(&mgr, str_adb.c_str(), str_config.c_str());
    if (!mgr) {
                                    ...
                                    const json::value& adb_config) const
{
    ...
    std::string str_config = adb_config.to_string();
    // TODO: fix as is
    std::shared_ptr<MAA_ADB_CTRL_UNIT_NS::ConnectionAPI> mgr;
    MAA_ADB_CTRL_UNIT_NS::create_connection(&mgr, str_adb.c_str(), serial.c_str(), 0, str_config.c_str());
    if (!mgr) {

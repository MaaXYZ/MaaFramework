...
std::shared_ptr<ChildPipeIOStream> InvokeApp::invoke_bin(const std::string& extra)
{
    ...
    ChildPipeIOStream ios(argv_opt->exec, argv_opt->args);
    using namespace std::chrono_literals;
    std::string output = ios.read(20s);

    LogInfo << invoke_bin_argv_.gen(argv_replace_);
    auto output_opt = io_ptr_->interactive_shell(invoke_bin_argv_.gen(argv_replace_), wants_stderr);
...
std::shared_ptr<ChildPipeIOStream> InvokeApp::invoke_app(const std::string& package)
{
    ...
    auto output_opt = io_ptr_->interactive_shell(invoke_app_argv_.gen(argv_replace_), false);

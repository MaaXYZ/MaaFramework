...
class GamescopeControlUnitMgr : public GamescopeControlUnitAPI
{
public:
    GamescopeControlUnitMgr(uint32_t node_id, std::string eis_socket_path, bool use_win32_vk_code);
    virtual ~GamescopeControlUnitMgr() override;
...
private:
    ...
    std::unique_ptr<PipeWireScreencap> screencap_;
    std::unique_ptr<EiInput> input_;
    std::string eis_socket_path_;
    uint32_t node_id_;
    bool use_win32_vk_code_;
};

...
std::optional<cv::Mat> StretchBltScreencap::screencap()
{
    if (!hwnd_) {
    ...
    // 使用StretchBlt而不是BitBlt（1:1缩放，效果相同但使用不同的API路径）
    if (!StretchBlt(mem_dc, 0, 0, width, height, hdc, 0, 0, width, height, SRCCOPY)) {
        LogError << "StretchBlt failed, error code: " << GetLastError();
        return std::nullopt;
    }

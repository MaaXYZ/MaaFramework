...
POINT PostMessageWithCursorPosInput::client_to_screen(int x, int y)
{
    POINT point = { x, y };
    if (hwnd_) {
...
void PostMessageWithCursorPosInput::save_cursor_pos()
{
    GetCursorPos(&saved_cursor_pos_);
    cursor_pos_saved_ = true;
}

void PostMessageWithCursorPosInput::restore_cursor_pos()
{
...
void PostMessageWithCursorPosInput::set_cursor_to_client_pos(int x, int y)
{
    POINT screen_pos = client_to_screen(x, y);
    SetCursorPos(screen_pos.x, screen_pos.y);
}

bool PostMessageWithCursorPosInput::touch_down(int contact, int x, int y, int pressure)
{
...
bool PostMessageWithCursorPosInput::touch_move(int contact, int x, int y, int pressure)
{
    std::ignore = pressure;
...
bool PostMessageWithCursorPosInput::touch_up(int contact)
{
    LogInfo << VAR(contact);
...
bool PostMessageWithCursorPosInput::scroll(int dx, int dy)
{

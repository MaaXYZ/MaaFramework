...
private:
    ...
    std::atomic<int> tracking_x_{ 0 };
    std::atomic<int> tracking_y_{ 0 };

    // 钩子存储的待处理鼠标位置（由 60fps 批处理消费）
    std::atomic<int> pending_mouse_x_{ 0 };
    std::atomic<int> pending_mouse_y_{ 0 };
    std::atomic<bool> has_pending_mouse_{ false };

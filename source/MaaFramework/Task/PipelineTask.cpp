...
NodeDetail PipelineTask::run_next(const PipelineData::NextList& list, const PipelineData& pretask)
{
    ...
    if (debug_mode() || !cur_node.focus.is_null()) {
        notify(MaaMsg_Node_Node_Starting, node_cb_detail);
    }
    ...
    while (!context_->need_to_stop()) {
        ...
        if (!hit_opt) {
            ...
            if (debug_mode() || !cur_node.focus.is_null()) {
                notify(MaaMsg_Node_Node_Failed, node_cb_detail);
            }
        ...
        if (debug_mode() || !cur_node.focus.is_null()) {
            notify(act.success ? MaaMsg_Node_Node_Succeeded : MaaMsg_Node_Node_Failed, node_cb_detail);
        }

...
NeuralNetworkDetector::ResultsVec NeuralNetworkDetector::detect(const std::vector<std::string>& labels, const ModelIOInfo& io_info) const
{
    ...
    try {
        ...
        LogError << "ORT detect failed" << VAR(name_) << VAR(e.what());
        return {};
    }
    catch (const std::exception& e) {
        LogError << "Detect failed" << VAR(name_) << VAR(e.what());
        return {};
    }
}
...
std::vector<std::string> NeuralNetworkDetector::parse_labels_from_metadata() const
{
    ...
    try {
        ...
        LogError << "Failed to parse labels from ORT metadata" << VAR(name_) << VAR(e.what());
        return {};
    }
    catch (const std::exception& e) {
        LogError << "Unexpected error while parsing model metadata" << VAR(name_) << VAR(e.what());
        return {};
    }
}

...
std::optional<RuntimeParam::Task> Configurator::generate_runtime_task(const Configuration::Task& config_task) const
{
    ...
    RuntimeParam::Task runtime_task { .name = data_task.name, .entry = data_task.entry, .pp_override = data_task.pp_override };
    ...
    for (const auto& [config_option, config_option_value] : config_task.option) {
        ...
        runtime_task.pp_override = data_case.pp_override | std::move(runtime_task.pp_override);
    }

...
**这是一个非官方的、以学习为目的的个人 fork 版本，请谨慎使用。**

本项目 fork 自优秀的开源项目 [MaaXYZ/MaaFramework](https://github.com/MaaXYZ/MaaFramework)。

## 主要修改

相较于原版，此 fork 新增了基于 `GetCursorPos` API Hook 的 Win32 后台点击功能，此功能仅为个人学习和研究目的实现。

## 使用方法

本项目与 MaaFramework 的集成方式完全一致。在您的打包或集成流程中，仅需将相关仓库链接替换为本仓库地址即可。

当输入方式（Input Method）配置为 SendMessage 时，系统会自动触发 Hook 机制并替换底层的点击实现。此过程对业务层透明，无需修改任何业务逻辑代码。

## 重要风险警告

**使用 Hook 功能具有极高的安全风险！**

* **极易导致游戏封号**：绝大多数游戏的反作弊系统都会检测并封禁使用此类技术的账号。
* **请勿用于任何生产或重要环境**。

**强烈不建议在不完全理解其原理和后果的情况下使用本仓库的任何内容。如果你因此造成任何损失，包括但不限于账号封禁，需自行承担全部责任。**

## 官方渠道

如需获取官方最新、最稳定的版本，或向原作者提交 Issue，请访问：

👉 **[MaaFramework 原始项目仓库](https://github.com/MaaXYZ/MaaFramework)** 👈
